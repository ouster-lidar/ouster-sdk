...
Sha256 Sha256::hash_files(const std::vector<std::pair<std::istream&, size_t>>& files,
                          std::pair<const void*, size_t> prefix) {
    ...
    std::memcpy(static_cast<void*>(hash_array.data()), hash_bytes.data(), sizeof(hash_array));
    return Sha256(hash_array);
}

Sha256 Sha256::hash_blobs(const std::vector<std::pair<const void*, size_t>>& blobs) {
    std::array<unsigned char, 32> hash_bytes{};
    EVP_MD_CTX* mdctx = EVP_MD_CTX_new();
