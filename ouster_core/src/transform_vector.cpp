...
#include "ouster/core/impl/transform_quaternion.h"
...
RotH RotV::exp(double& angle_out, double& sin_angle_out, double& cos_angle_out) const {
    angle_out = norm();
...
RotQ RotV::q(double& angle, double& sin_angle, double& cos_angle) const {
    ...
    if (std::abs(angle) > EPS) {
        quat = RotQ(cos_angle_2, sin_angle_2 * operator()(0) / angle,
                    sin_angle_2 * operator()(1) / angle, sin_angle_2 * operator()(2) / angle);
    }
    return quat;
}
