...
Zip::Zip(const uint8_t* data, size_t size, int flags) {
    ...
    if (source_ == nullptr) {
        ...
        string_stream << "Error reading zip blob: " << zip_error_strerror(&error_);
    ...
    if (archive_ == nullptr) {
        std::stringstream string_stream;
        string_stream << "Error reading zip source: " << zip_error_strerror(&error_);
        zip_error_fini(&error_);
...
Zip::Zip() : Zip(nullptr, 0, 0) {  // NOLINT(cppcoreguidelines-pro-type-member-init)
}
...
std::vector<uint8_t> Zip::get_file(const std::string& filename) {
    ...
    zip_int64_t index = zip_name_locate(archive_, filename.c_str(), ZIP_FL_NOCASE);
    if (index < 0) {
    ...
    if (out.empty()) {
        throw std::runtime_error("Zip contained " + filename + ", but it is empty");
    }
    return out;
}
...
void Zip::add_file(const std::string& name, const std::vector<uint8_t>& data) {
    ...
    if (size == 0) {
        throw std::runtime_error("Cannot add empty file to zip archive: " + name);
    }
    ...
    if (zip_file_add(archive_, name.c_str(), src, ZIP_FL_ENC_GUESS) < 0) {
        ...
        std::stringstream string_stream;
        string_stream << "Error adding \"" << name
                      << "\" zip file to archive: " << zip_error_strerror(error);
