...
struct Logger::internal_logger {
    ...
    internal_logger(const std::string& logger_name, std::string pattern)
        : logger_(std::make_unique<spdlog::logger>(
              logger_name, std::make_shared<spdlog::sinks::stderr_sink_mt>())),
          pattern_(std::move(pattern)) {}

    void configure_generic_sink(spdlog::sink_ptr sink, const std::string& log_level) {
...
Logger::Logger()
    : internal_logger_(std::make_unique<Logger::internal_logger>(LOGGER_NAME, LOGGER_PATTERN)) {
...
bool Logger::configure_stdout_sink(const std::string& log_level) {
    try {
        internal_logger_->configure_generic_sink(std::make_shared<spdlog::sinks::stdout_sink_mt>(),
                                                 log_level);
    } catch (const spdlog::spdlog_ex& ex) {
        std::cerr << LOGGER_NAME << " init_logger failed: " << ex.what() << std::endl;
        return false;
    }
...
bool Logger::configure_stderr_sink(const std::string& log_level) {
    try {
        internal_logger_->configure_generic_sink(std::make_shared<spdlog::sinks::stderr_sink_mt>(),
                                                 log_level);
    } catch (const spdlog::spdlog_ex& ex) {
        std::cerr << LOGGER_NAME << " init_logger failed: " << ex.what() << std::endl;
        return false;
...
bool Logger::configure_file_sink(const std::string& log_level, const std::string& log_file_path,
...
std::shared_ptr<Logger::format_builder> Logger::make_builder(const std::string& format_string) {
    return std::make_shared<Logger::format_builder>(format_string);
}

struct Logger::format_builder {
    ~format_builder() = default;
    format_builder(const std::string& format_string) : format_string(format_string){};
...
std::string Logger::finalize_format_builder(
    const std::shared_ptr<Logger::format_builder>& builder) {
...
void Logger::enable_auto_newline() {
    internal_logger_->enable_auto_newline();
}

#define LOGGER_PROCESS_ARG(SINGLE_LOGGER_PROCESS_ARG_type)                                    \
    template void Logger::process_arg(const std::shared_ptr<Logger::format_builder>& builder, \
...
template <typename T>
void Logger::process_arg(const std::shared_ptr<Logger::format_builder>& builder, T data) {
