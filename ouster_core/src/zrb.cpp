...
Zrb::Zrb(uint32_t n_rows, uint32_t n_cols, float m_per_zmbin, uint64_t serial_number_init,
         Sha256 stl_hash_init, mat4d beam_to_lidar_init, mat4d lidar_to_sensor_init,
         mat4d sensor_to_body_init)
    : near_range_mm{img_t<uint32_t>(n_rows, n_cols)},
      far_range_mm{img_t<uint32_t>(n_rows, n_cols)},
      stl_hash(stl_hash_init),
      beam_to_lidar_transform(std::move(beam_to_lidar_init)),
      lidar_to_sensor_transform(std::move(lidar_to_sensor_init)),
      sensor_to_body_transform(std::move(sensor_to_body_init)),
...
void Zrb::save(std::ostream& out) const {
    ...
    if (beam_to_lidar_transform.isZero() || lidar_to_sensor_transform.isZero() ||
    ...
    CacheHeader header(*this, valid_cols, serial_number_from_int(serial_number));

    // Calculate bounds hash
    auto bounds_hash =
        Sha256::hash_blobs({{zr_buffer.data(), zr_buffer.size() * sizeof(uint32_t)}});
    header.meta.bounds_hash = bounds_hash;

    // Calculate total hash (excludes transforms)
    constexpr size_t metadata_hash_size = offsetof(CacheRenderMetadata, beam_to_lidar);
    auto total_hash = Sha256::hash_blobs({{&header.meta, metadata_hash_size}});
    header.info.hash = total_hash;

    out.write(reinterpret_cast<const char*>(&header), sizeof(header));
    out.write(reinterpret_cast<const char*>(zr_buffer.data()),
...
OUSTER_API_FUNCTION bool operator==(const Zrb& lhs, const Zrb& rhs) {
    return (lhs.near_range_mm == rhs.near_range_mm).all() &&
           (lhs.far_range_mm == rhs.far_range_mm).all() && lhs.m_per_zmbin_ == rhs.m_per_zmbin_ &&
           lhs.stl_hash == rhs.stl_hash && lhs.serial_number == rhs.serial_number &&
...
bool Zrb::read_zone_cache(std::istream& file, bool load_bounds,
    ...
    if (bounds_hash != header.meta.bounds_hash) {
        std::cerr << "ZoneCache: Bounds hash mismatch: " << bounds_hash.str() << " vs "
                  << header.meta.bounds_hash.str() << "\n";
    ...
    if (hash != header.info.hash) {
        std::cerr << "ZoneCache: Hash mismatch: " << hash.str() << " vs " << header.info.hash.str()
                  << "\n";
    ...
    auto serial_number_str =
        std::string(header.meta.serial_number.data(), header.meta.serial_number.size());
    serial_number = std::stoll(serial_number_str);
    valid_col_mask = header.meta.valid_col_mask;

    // Load transforms from header
    for (int i = 0; i < 16; ++i) {
        beam_to_lidar_transform.data()[i] = static_cast<double>(header.meta.beam_to_lidar[i]);
        lidar_to_sensor_transform.data()[i] = static_cast<double>(header.meta.lidar_to_sensor[i]);
        sensor_to_body_transform.data()[i] = static_cast<double>(header.meta.sensor_to_body[i]);
    }
    ...
    if (load_bounds) {
        ...
        for (uint32_t col_id = 0; col_id < header.meta.n_cols; col_id++) {
            ...
            if (!file.read(reinterpret_cast<char*>(col_buffer.data()),
                           static_cast<std::streamsize>(sizeof(uint32_t) * header.meta.n_rows))) {
