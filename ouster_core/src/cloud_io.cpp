...
PointCloudXYZf read_pointcloud(const std::string& filename) {
            ...
            if (info.format == "ply") {
                ...
                if (cmd == "element") {
                    if (words.size() == 3 && words[1] == "vertex") {
                        ...
                        throw std::runtime_error("Unsupported element: " + line);
                    }
                } else if (cmd == "property") {
                    if (words.size() == 3) {
                        if (field_sizes.count(words[1]) == 0) {
                            throw std::runtime_error("Unsupported type: " + words[1]);
                        }
                        Field field{words[2], words[1], current_offset, field_sizes[words[1]]};
                        fields.push_back(field);
                        current_offset += field.size;
                    } else {
                        throw std::runtime_error("Unsupported property: " + line);
                    }
                } else if (cmd == "format") {
                    if (words.size() == 3 && words[1] == "ascii") {
                        is_binary = false;
                    } else if (words.size() == 3 && words[1] == "binary_little_endian") {
                        is_binary = true;
                    } else {
                ...
                if (cmd == "DATA") {
                    if (words.size() == 2) {
                        ...
                        throw std::runtime_error("Invalid DATA property: " + line);
                    }
                } else if (cmd == "FIELDS") {
                    ...
                    for (size_t i = 1; i < words.size(); ++i) {
                        ...
                        if (sz != 4) {
                            throw std::runtime_error("Unsupported size: " + words[i]);
                        }
                        fields[i - 1].size = sz;
                        fields[i - 1].offset = offset_accum;
                        offset_accum += sz;
                    }
                    current_offset = offset_accum * 1;  // store total size per point
                } else if (cmd == "TYPE") {
                    if (words.size() - 1 != fields.size()) {
                    ...
                    for (size_t i = 1; i < words.size(); ++i) {
                        if (words[i] != "F") {
                            throw std::runtime_error("Unsupported type: " + words[i]);
                        }
                    ...
                    if (words.size() == 2) {
                        ...
                        throw std::runtime_error("Unsupported POINTS format: " + line);
                    }
                }
            }
        }
    ...
    auto read_ascii_data = [&](std::ifstream& input, const HeaderInfo& info) {
        ...
        for (auto& field : info.fields) {
            ...
            column_to_element.push_back((it != field_to_index.end()) ? it->second : -1);
        }
        int i = 0;
        std::string line;
        while (getline2(input, line) && i < info.num_vertices) {
            auto words = split(line, " ");
            for (size_t w = 0; w < words.size() && w < column_to_element.size(); w++) {
                if (column_to_element[w] >= 0) {
                    pts[(i * 3) + column_to_element[w]] =
                        static_cast<float>(std::atof(words[w].c_str()));
                }
            }
            i++;
        }
