...
int main(int argc, char* argv[]) {
    ...
    nonstd::optional<mat4d> sensor_to_body_transform;
    if (zone_monitor_json.contains("sensor_to_body_transform")) {
        sensor_to_body_transform =
            mat4d_from_array(zone_monitor_json["sensor_to_body_transform"]
    ...
    Zone zone;
    zone.point_count = zone_monitor_json["zones"]["0"]["point_count"].as<uint16_t>();
    zone.frame_count = zone_monitor_json["zones"]["0"]["frame_count"].as<uint16_t>();
    Zone::string_to_zonemode(zone_monitor_json["zones"]["0"]["mode"].as<std::string>(), zone.mode);
    auto stl = Stl(stl_path);

    Stl::string_to_coordinate_frame(
        zone_monitor_json["zones"]["0"]["stl"]["coordinate_frame"].as<std::string>(),
        stl.coordinate_frame);
    zone.stl = stl;
    if (zone.render(beam_config)) {
        zone.zrb->save(zrb_path);
    } else {
        std::cerr << "ZoneMon: render failed or mesh was entirely out of view.\n";
    }
}
