...
std::array<float, 16> canonicalize_transform(const mat4d& transform) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        double rounded = std::round(value / TRANSFORM_HASH_PRECISION) * TRANSFORM_HASH_PRECISION;
...
// Cache definitions
CacheRenderMetadata::CacheRenderMetadata(const Zrb& zrb,
    ...
    if (zrb.beam_to_lidar_transform.isZero()) {
        throw std::logic_error("CacheRenderMetadata: beam_to_lidar_transform not set");
    }
    if (zrb.lidar_to_sensor_transform.isZero()) {
        throw std::logic_error("CacheRenderMetadata: lidar_to_sensor_transform not set");
    }
    if (zrb.sensor_to_body_transform.isZero()) {
        throw std::logic_error("CacheRenderMetadata: sensor_to_body_transform not set");
    }
}

CacheHeader::CacheHeader(const Zrb& zrb, const std::bitset<2048>& valid_col_mask_init,
                         SerialNumber serial_number_init)
