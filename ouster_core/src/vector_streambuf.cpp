...
VectorStreamBuf::VectorStreamBuf(const ByteVector* buffer)
    ...
    : buffer_(const_cast<ByteVector*>(buffer)) {  // std::streambuf's setg and setp methods require
                                                  // non-const pointer args.
    if (buffer_ == nullptr) {
        throw std::invalid_argument("VectorStreamBuf: buffer pointer is null");
    }
    setg(reinterpret_cast<char*>(buffer_->data()), reinterpret_cast<char*>(buffer_->data()),
...
std::streampos VectorStreamBuf::seekoff(std::streamoff off, std::ios_base::seekdir way,
...
std::streampos VectorStreamBuf::seekpos(std::streampos position, std::ios_base::openmode openmode) {
