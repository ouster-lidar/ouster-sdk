...
RotV RotH::log(double& angle_out, double& cos_angle_out) const {
    cos_angle_out = 0.5 * (operator()(0, 0) + operator()(1, 1) + operator()(2, 2) - 1.0);
    cos_angle_out = std::max(cos_angle_out, -1.0 + EPS);
    cos_angle_out = std::min(cos_angle_out, 1.0 - EPS);
    angle_out = std::acos(cos_angle_out);
    RotV rot_v(operator()(2, 1) - operator()(1, 2), operator()(0, 2) - operator()(2, 0),
...
PoseV PoseH::log(double& angle_out, double& sin_angle_out, double& cos_angle_out) const {
    PoseV vec;
    vec.set_rot(r().log(angle_out, cos_angle_out));
    sin_angle_out = std::sin(angle_out);
    vec.set_trans(vec.r().vee(angle_out, sin_angle_out, cos_angle_out).inverse() * t());
    return vec;
}
