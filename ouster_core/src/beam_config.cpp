...
BeamConfig::BeamConfig(uint32_t n_cols_init, const std::vector<double>& px_altitudes_init,
                       const std::vector<double>& px_azimuths_init,
                       mat4d beam_to_lidar_transform_init, mat4d lidar_to_sensor_transform_init,
                       nonstd::optional<mat4d> sensor_to_body_transform_init,
    ...
    : n_cols(n_cols_init),
      ...
      lut(sensor_to_body_transform
              ? std::make_shared<XYZLut>(impl::make_xyz_lut(
                    n_cols, n_rows, 0.001, beam_to_lidar_transform,
                    scale_translation(*sensor_to_body_transform) * lidar_to_sensor_transform,
                    px_azimuths, px_altitudes))
              : nullptr),
      lut_no_sensor_to_body_transform(std::make_shared<XYZLut>(
          impl::make_xyz_lut(n_cols, n_rows, 0.001, beam_to_lidar_transform,
...
bool BeamConfig::operator==(BeamConfig const& rhs) const {
    return (n_cols == rhs.n_cols) && (n_rows == rhs.n_rows) && (px_altitudes == rhs.px_altitudes) &&
           ...
           (m_per_zmbin == rhs.m_per_zmbin) && (serial_number == rhs.serial_number);
}
