...
#include "ouster/core/zone_lut.h"
...
std::pair<Coord, float> compute_bounding_sphere(const std::vector<Triangle>& triangles) {
    ...
    for (auto& tri : triangles) {
        radius_squared = std::max({(tri.coords[0] - centroid).squaredNorm(),
                                   ...
                                   (tri.coords[2] - centroid).squaredNorm(), radius_squared});
    }
    return {centroid, std::sqrt(radius_squared)};
}

std::string trim_whitespace(const std::string& str, const std::string& whitespace = " \t\r") {
...
void log_parsing_error(const std::string& message, const std::string& line = "") {
...
bool load_from_stl_ascii_facet(std::istream& stl_file, Triangle& out_triangle) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (!read_stl_ascii_line(stl_file, line) || !std::regex_search(line, match, vertex_re) ||
            match.size() != 4) {
            log_parsing_error("Expected 'vertex'", line);
            return false;
        }
        vertices.at(i) = Coord(std::stof(match[1]), std::stof(match[2]), std::stof(match[3]));
    }
...
Mesh::Mesh(std::vector<Triangle>&& tris)
    : triangles_{std::move(tris)}, bounding_sphere_{compute_bounding_sphere(triangles_)} {}
...
bool Mesh::load_from_stl_ascii(std::istream& stl_file) {
    ...
    if (!read_stl_ascii_line(stl_file, line) || !std::regex_search(line, solid_re)) {
    ...
    while (read_stl_ascii_line(stl_file, line)) {
        if (std::regex_search(line, facet_re)) {
            Triangle new_triangle(Coord{0, 0, 0}, Coord{0, 0, 0}, Coord{0, 0, 0});
...
bool Mesh::load_from_stl_binary(std::istream& input_stream) {
    ...
    for (uint32_t i = 0; i < n_tris; i++) {
        if (!input_stream.read(reinterpret_cast<char*>(&in_tri), sizeof(in_tri))) {
        ...
        input_stream.read(reinterpret_cast<char*>(&attr_b_cnt), sizeof(attr_b_cnt));
    }
    triangles_ = tris;
    bounding_sphere_ = compute_bounding_sphere(triangles_);
    return true;
...
bool Mesh::load_from_stl_stream(std::istream& input_stream) {
    // if the file contains the phrase "endsolid" after the header then we
    // assume it's ascii, otherwise binary.
    std::string::size_type endsolid_pos = std::string::npos;
    try {
        endsolid_pos = impl::kmp_search(input_stream, "endsolid");
    } catch (const std::runtime_error& e) {
        log_parsing_error("Error during STL format detection: " + std::string(e.what()));
        return false;
    }
    bool stl_is_ascii = endsolid_pos > HEADER_BYTES && endsolid_pos != std::string::npos;
    input_stream.clear();
    input_stream.seekg(0, std::ios::beg);

    if (input_stream.fail()) {
        log_parsing_error(
            "Failed to seek to the beginning of the stream after format "
            "detection.");
        return false;
    }
...
bool Mesh::intersects_with_bounding_sphere(const Ray& beam) const {
    ...
    float c = centroid_to_beam_origin.dot(centroid_to_beam_origin) - (radius * radius);
...
void Mesh::save_stl_binary(std::ostream& output_stream) const {
    std::array<char, HEADER_BYTES> header{};
    output_stream.write(header.data(), HEADER_BYTES);

    auto& tris = this->triangles();
    uint32_t n_tris = tris.size();
    output_stream.write(reinterpret_cast<char*>(&n_tris), sizeof(n_tris));

    for (const auto& tri : tris) {
        StlTri out_tri{};
        out_tri.v0 = {tri.coords[0][0], tri.coords[0][1], tri.coords[0][2]};
        out_tri.v1 = {tri.coords[1][0], tri.coords[1][1], tri.coords[1][2]};
        out_tri.v2 = {tri.coords[2][0], tri.coords[2][1], tri.coords[2][2]};
        out_tri.normal = {tri.normal[0], tri.normal[1], tri.normal[2]};
        output_stream.write(reinterpret_cast<char*>(&out_tri), sizeof(out_tri));
        uint16_t attr_b_cnt = 0;
        output_stream.write(reinterpret_cast<char*>(&attr_b_cnt), sizeof(attr_b_cnt));
    }
}

void Mesh::save_stl_binary(const std::string& path) const {
    std::ofstream output_stream(path, std::ios::out | std::ios::binary);
    if (!output_stream) {
        throw std::runtime_error("Failed to open file for writing: " + path);
    }
    save_stl_binary(output_stream);
}
