...
std::vector<size_t> calculate_strides(const std::vector<size_t>& shape) {
    auto total = std::accumulate(shape.begin(), shape.end(), size_t{1}, std::multiplies<>{});
...
extern OUSTER_API_VAR const Table<FieldClass, const char*, 5> FIELD_CLASS_STRINGS{
    {{FieldClass::PIXEL_FIELD, "PIXEL_FIELD"},
     ...
     {FieldClass::FRAME_FIELD, "FRAME_FIELD"},
     {FieldClass::COLLATION_FIELD, "COLLATION_FIELD"}}};

}  // namespace impl

std::string to_string(FieldClass flag) {
    auto end = impl::FIELD_CLASS_STRINGS.end();
    auto res = std::find_if(impl::FIELD_CLASS_STRINGS.begin(), end,
...
size_t FieldDescriptor::bytes() const {
...
bool FieldDescriptor::is_type_compatible(const FieldDescriptor& other) const noexcept {
    return (type == 0u) || (other.type == 0u) || type == other.type;
}

size_t FieldDescriptor::ndim() const noexcept {
    return shape.size();
}

FieldView::FieldView() noexcept : ptr_(nullptr), desc_() {}

FieldView::FieldView(void* ptr, FieldDescriptor desc) : ptr_(ptr), desc_(std::move(desc)) {}

FieldView::operator bool() const noexcept {
    return !(get() == nullptr);
}

size_t FieldView::bytes() const noexcept {
    return desc_.bytes();
}

size_t FieldView::size() const {
    return desc_.size();
}
...
const FieldDescriptor& FieldView::desc() const {
    return desc_;
}

const std::vector<size_t>& FieldView::shape() const {
    return desc_.shape;
}

ChanFieldType FieldView::tag() const {
    return desc_.tag();
}
...
bool FieldView::sparse() const {
    ...
    for (int i = 1, end = static_cast<int>(shape.size()); i < end; ++i) {
        dense = dense && (strides[i - 1] == strides[i] * shape[i]);
    }
    return !dense;
}

void FieldView::set_zero() {
    switch (tag()) {
        ...
        default:
            if (sparse()) {
                throw std::logic_error(
                    "FieldView::set_zero: cannot set zero on a sparse view of "
                    "unknown type");
            }
...
Field::Field() noexcept : FieldView(), class_{FieldClass::FRAME_FIELD} {}
Field::~Field() {
    free(ptr_);
}
...
Field::Field(Field&& other) noexcept : Field() {
    swap(other);
};
...
Field::Field(const Field& other) : FieldView(nullptr, other.desc()), class_{other.class_} {
...
FieldClass Field::field_class() const {
    return class_;
}
...
bool Field::operator==(const Field& other) const {
    return matches(other.desc()) && (std::memcmp(ptr_, other.ptr_, bytes()) == 0) &&
           class_ == other.class_;
}

FieldView uint_view(const FieldView& other) {
    if (other.shape().empty()) {
        throw std::invalid_argument("uint_view: attempted converting a non-array FieldView");
    }
    ...
    switch (other.desc().element_size) {
        ...
        default:
            ...
            throw std::invalid_argument("uint_view: got wrong element size " +
...
Field destagger(const SensorInfo& info, const FieldView& field, bool inverse) {
    Field result(field.desc(), FieldClass::PIXEL_FIELD);

    auto destagger_fn = [&](auto ref) {
        destagger_into<std::decay_t<decltype(ref(0))>>(ref, info.format.pixel_shift_by_row, inverse,
                                                       result);
    };
