...
class MultiFrameSetSource;
class MultiFrameSetIteratorImpl : public FrameSetIteratorImpl {
   public:
    explicit MultiFrameSetIteratorImpl(const MultiFrameSetSource* frame_set_source,
                                       int sensor_idx = -1)
        : source_(frame_set_source) {
        // NOLINTNEXTLINE(cppcoreguidelines-prefer-member-initializer)
        sensor_idx_ = sensor_idx < 0 ? -1 : sensor_idx;

        // todo try not to decode frames in the constructor
        for (size_t i = 0; i < frame_set_source->sources_.size(); i++) {
            iters_[i] = frame_set_source->sources_[i]->begin();
            if (iters_[i] != frame_set_source->sources_[i]->end()) {
                nexts_[i] = std::make_unique<FrameSet>(*iters_[i]);
            } else {
                iters_.erase(i);
            }
        }
    ...
    bool advance(size_t offset) override {
        ...
        for (size_t i = 0; i < offset; i++) {
            ...
            if (offset > 1 && source_->is_indexed() && sensor_idx_ < 0) {
                ...
                // the fun for this is we need to know which frame set source
                // each thing came from so will need _another_ index for this
                ...
                // Get the source of the next frame
                int target = source_->source_index_[position_ + offset].source_index;

                for (const auto& amt : increment_amounts) {
                    iters_[amt.first] += amt.second;
                    if (iters_[amt.first] == source_->sources_[amt.first]->end()) {
                        nexts_.erase(amt.first);
                        iters_.erase(amt.first);
                    } else {
                        nexts_[amt.first] = std::make_unique<FrameSet>(*iters_[amt.first]);
                    }
                }

                // then just grab the value from the target iterator and update
                // their nexts
                frame_set_ = std::move(nexts_[target]);
                nexts_[target].reset();

                if (frame_set_->size() > 1) {
                    throw std::runtime_error(
                        "MultiFrameSetSource can only be used with "
                        "non-collated "
                        "sources.");
                }

                // update sensor info
                auto res = source_->info_map_.find((*frame_set_)[0]->sensor_info.get());
                (*frame_set_)[0]->sensor_info = res->second.second;

                position_ += static_cast<int64_t>(offset);
                return false;
            }
            ...
            for (auto& item : nexts_) {
                ...
                nexts_[item.first] = std::make_unique<FrameSet>(*iter);
            }
            ...
            for (const auto& item : nexts_) {
                ...
                auto timestamp = (*item.second)[0]->get_first_valid_packet_timestamp();
                if (timestamp < minimum_ts) {
                    earliest = static_cast<int>(item.first);
                    minimum_ts = timestamp;
                }
            }

            // "pop" off the frame
            frame_set_ = std::move(nexts_[earliest]);
            nexts_[earliest].reset();

            if (frame_set_->size() > 1) {
                throw std::runtime_error(
                    "MultiFrameSetSource can only be used with non-collated "
                    "sources.");
            }
            ...
            // skip frames from sensors we want to filter out
   ...
   private:
    const MultiFrameSetSource* source_;
    std::unique_ptr<FrameSet> frame_set_;
    int sensor_idx_ = -1;

    int64_t position_ = -1;

    std::map<size_t, FrameSetIterator> iters_;
    std::map<size_t, std::unique_ptr<FrameSet>> nexts_;
};

MultiFrameSetSource::MultiFrameSetSource(std::vector<std::shared_ptr<FrameSetSource>> sources)
    : sources_(std::move(sources)), indexed_(true), size_(0) {
    ...
    // if we are indexed, build our massive indices and calculate size and
    // frames num
    if (!indexed_) {
        size_hint_ = 0;
        for (const auto& src : sources_) {
    ...
    frames_num_.resize(sensor_info_.size(), 0);
    for (size_t src_index = 0; src_index < sources_.size(); src_index++) {
        ...
        for (size_t i = 0; i < src->sensor_info().size(); i++) {
            ...
            frames_num_[sensor_index] += src->frames_num()[i];
        ...
        for (size_t i = 0; i < f_idx.size(); i++) {
            ...
            full_index_.emplace_back(sample.first, src_idx_to_dest_index[sample.second]);

            IndexSample sample2;
            sample2.source_index = static_cast<int>(src_index);
            sample2.timestamp = sample.first;
            sample2.frame_index = static_cast<int>(i);
    ...
    std::sort(source_index_.begin(), source_index_.end(),
              [](const IndexSample& a, const IndexSample& b) { return a.timestamp < b.timestamp; });
    ...
    for (uint64_t i = 0; i < full_index_.size(); i++) {
        ...
        individual_index_[sample.second].emplace_back(sample.first, i);
    }
}

FrameSetIterator MultiFrameSetSource::begin() const {
    return FrameSetIterator(this, new MultiFrameSetIteratorImpl(this));
}

FrameSetIterator MultiFrameSetSource::begin(int idx) const {
    return FrameSetIterator(this, new MultiFrameSetIteratorImpl(this, idx));
}

FrameSetIterator MultiFrameSetSource::end() const {
    return FrameSetIterator(this);
}

const std::vector<std::shared_ptr<SensorInfo>>& MultiFrameSetSource::sensor_info() const {
    return sensor_info_;
}

bool MultiFrameSetSource::is_live() const {
    return false;
}

bool MultiFrameSetSource::is_indexed() const {
    return indexed_;
}

size_t MultiFrameSetSource::size() const {
    return size_;
}

const std::vector<std::vector<std::pair<uint64_t, uint64_t>>>&
MultiFrameSetSource::individual_index() const {
    if (!is_indexed()) {
        throw std::runtime_error(
...
const std::vector<std::pair<uint64_t, uint64_t>>& MultiFrameSetSource::full_index() const {
    if (!is_indexed()) {
...
size_t MultiFrameSetSource::size_hint() const {
    return size_hint_;
}

const std::vector<size_t>& MultiFrameSetSource::frames_num() const {
    if (!is_indexed()) {
...
std::unique_ptr<FrameSetSource> MultiFrameSetSource::move() {
    return std::make_unique<MultiFrameSetSource>(std::move(*this));
}

void MultiFrameSetSource::close() {
    sources_.clear();
}
