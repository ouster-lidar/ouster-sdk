...
#include "ouster/core/xyzlut.h"  // NOLINT(unused-includes)
...
void Zone::check_invariants() const {
    ...
    if (stl) {
        ...
        if (stl->coordinate_frame == Stl::CoordinateFrame::NONE) {
            throw std::logic_error("Zone: STL coordinate frame must be BODY or SENSOR");
        }
    }
    if (zrb) {
...
bool Zone::render(const BeamConfig& config) {
    ...
    if (mesh.triangles().size() > MAX_TRIANGLES) {
        std::cerr << "Zone: Error rendering zone, STL has too many triangles.\n";
        return false;
    }

    if (stl->coordinate_frame == Stl::CoordinateFrame::BODY &&
        !config.sensor_to_body_transform.has_value()) {
        logger().error(
            "Zone: Error rendering zone, sensor_to_body_transform not set for "
            "BODY coordinate frame.");
        return false;
    }

    const mat4d sensor_to_body_for_zrb =
        config.sensor_to_body_transform.value_or(mat4d::Identity());
    zrb = Zrb(config.n_rows, config.n_cols, config.m_per_zmbin, config.serial_number, stl->hash(),
              config.beam_to_lidar_transform, config.lidar_to_sensor_transform,
              sensor_to_body_for_zrb);

    // Pre-select LUT references to avoid branch in inner loop
    const auto& beam_offsets = (stl->coordinate_frame == Stl::CoordinateFrame::BODY)
                                   ? config.lut->offset
                                   : config.lut_no_sensor_to_body_transform->offset;
    const auto& beam_directions = (stl->coordinate_frame == Stl::CoordinateFrame::BODY)
    ...
    for (uint32_t row = 0; row < config.n_rows; row++) {
        ...
        for (uint32_t col = 0; col < config.n_cols; col++) {
            ...
            beam.direction = beam_directions.row(pixel_offset).cast<float>() * 1000.0f;
    ...
    if (pixels_with_intersections > 0 && pixels_with_intersections < point_count) {
        throw std::logic_error("Zone: area of rendered zone (" +
                               std::to_string(pixels_with_intersections) +
                               ") is smaller than "
                               "point_count (" +
                               std::to_string(point_count) + ") specified in zone.");
    }
...
bool operator==(const Zone& lhs, const Zone& rhs) {
    return lhs.point_count == rhs.point_count && lhs.frame_count == rhs.frame_count &&
           lhs.mode == rhs.mode && lhs.stl == rhs.stl && lhs.zrb == rhs.zrb;
}
