...
ValidatorIssues::ValidatorEntry::ValidatorEntry(const std::string& path, const std::string& msg)
    : path_(path), msg_(msg) {}
...
jsoncons::json jsonpath_query(const jsoncons::json& root, const std::string& path) {
    return jsoncons::jsonpath::json_query(root, path);
}
...
void jsonpath_query(const jsoncons::json& root, const std::string& path,
    ...
    jsoncons::jsonpath::json_query(root, path, std::move(callback));
}
...
void JsonTools::skipped_due_to_item(ValidatorIssues::EntryList& severity,
    ...
    std::stringstream error_message;
    error_message << "Validation step for path: \"" << item_skipped << "\" skipped"
                  << " due to failures validating path: \"" << cause_item << "\"." << explanation;

    auto entry = ValidatorIssues::ValidatorEntry(item_skipped, error_message.str());
    severity.push_back(entry);
}

bool JsonTools::path_exists(const std::string& path) {
    return (!jsonpath_query(root_, path).empty());
}

// TODO[tws] this method should accept an array instead of a vector
void JsonTools::decode_transform_array(mat4d& output, const std::vector<double>& data) {
    if (data.size() != mat4d::SizeAtCompileTime) {
        throw std::invalid_argument("Transform array must have exactly " +
                                    std::to_string(mat4d::SizeAtCompileTime) + " elements.");
    }
