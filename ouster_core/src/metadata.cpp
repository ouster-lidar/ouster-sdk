...
#include "ouster/core/impl/json_tools.h"
...
extern double default_lidar_origin_to_beam_origin(const std::string& prod_line);
extern mat4d default_beam_to_lidar_transform(const std::string& prod_line);
...
class SensorInfoImpl : public impl::JsonTools {
   public:
    SensorInfoImpl(const jsoncons::json& root, SensorInfo& sensor_info, ValidatorIssues& issues)
   ...
   protected:
    SensorInfoImpl(const jsoncons::json& root, ValidatorIssues& issues) : JsonTools(root, issues) {}
     ...
     */
    const std::string pixels_per_column_string_{"$.lidar_data_format.pixels_per_column"};

    /**
     * Json path for the number of columns per frame.
     */
    const std::string columns_per_frame_string_{"$.lidar_data_format.columns_per_frame"};

    // Sections
    void parse_and_validate_sensor_info(SensorInfo& sensor_info) {
        parse_and_validate_datetime(issues_.information, "$.sensor_info.build_date", "%Y-%m-%dT%TZ",
                                    sensor_info.build_date);

        parse_and_validate_item(issues_.information, "$.sensor_info.build_rev", sensor_info.fw_rev,
        ...
        parse_and_validate_item(issues_.information, "$.sensor_info.initialization_id",
                                sensor_info.init_id);
        ...
        if (parse_and_validate_item(issues_.information, prod_line_string_,
            ...
            try {
                ...
                auto entry = ValidatorIssues::ValidatorEntry(prod_line_string_, error.what());
                issues_.warning.push_back(entry);
            }
        }

        parse_and_validate_item(issues_.information, "$.sensor_info.prod_pn", sensor_info.prod_pn,
                                verify_string_not_empty);

        std::string sn_string;
        parse_and_validate_item(issues_.information, "$.sensor_info.prod_sn", sn_string,
                                verify_string_not_empty);
        char* end = nullptr;
        ...
        if (end != expected_end) {
            ...
            auto entry =
                ValidatorIssues::ValidatorEntry("$.sensor_info.prod_sn", error_message.str());
            issues_.information.push_back(entry);
        }

        parse_and_validate_item(issues_.information, "$.sensor_info.status", sensor_info.status,
                                verify_string_not_empty);
    }

    void parse_and_validate_config_params(SensorConfig& config) {
        std::vector<uint64_t> azimuth_window_data;
        if (parse_and_validate_item(issues_.information, "$.config_params.azimuth_window.*",
                                    azimuth_window_data, 2,
                                    make_verify_in_bounds<uint64_t>(0, 360000))) {
            config.azimuth_window = {azimuth_window_data[0], azimuth_window_data[1]};
        }

        parse_and_validate_item(issues_.information, "$.config_params.lidar_frame_azimuth_offset",
                                config.lidar_frame_azimuth_offset,
                                make_verify_in_bounds<uint64_t>(0, 360000));

        parse_and_validate_item(issues_.information, "$.config_params.columns_per_packet",
                                config.columns_per_packet);

        if (parse_and_validate_enum<std::string>(issues_.information, lidar_mode_string_,
                                                 config.lidar_mode, lidar_mode_of_string)) {
        ...
        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.nmea_baud_rate",
                                             config.nmea_baud_rate, nmea_baud_rate_of_string);

        uint64_t nmea_ignore_valid_char = 0;
        if (parse_and_validate_item(issues_.information, "$.config_params.nmea_ignore_valid_char",
                                    nmea_ignore_valid_char)) {
            config.nmea_ignore_valid_char = (nmea_ignore_valid_char != 0);
        }

        parse_and_validate_enum<std::string>(issues_.information,
                                             "$.config_params.nmea_in_polarity",
                                             config.nmea_in_polarity, polarity_of_string);

        parse_and_validate_item(issues_.information, "$.config_params.nmea_leap_seconds",
                                config.nmea_leap_seconds, true);

        const std::string operating_mode_string = "$.config_params.operating_mode";
        if (!parse_and_validate_enum<std::string>(issues_.information, operating_mode_string,
                                                  config.operating_mode,
                                                  operating_mode_of_string)) {
            const std::string auto_start_flag_string = "$.config_params.auto_start_flag";
            ...
            std::string auto_start_flag_deprecation =
                ...
                "appropriately...";
            if (parse_and_validate_item<bool>(issues_.information, auto_start_flag_string,
                                              auto_start_flag)) {
                auto entry = ValidatorIssues::ValidatorEntry(auto_start_flag_string,
                                                             auto_start_flag_deprecation);
                issues_.information.push_back(entry);
                config.operating_mode =
                    auto_start_flag ? OperatingMode::NORMAL : OperatingMode::STANDBY;
            } else if (parse_and_validate_item<int>(issues_.information, auto_start_flag_string,
                                                    auto_start_int, true)) {
                auto entry = ValidatorIssues::ValidatorEntry(auto_start_flag_string,
                                                             auto_start_flag_deprecation);
                issues_.information.push_back(entry);
        ...
        parse_and_validate_item(issues_.information, "$.config_params.phase_lock_enable",
                                config.phase_lock_enable);

        parse_and_validate_item(issues_.information, "$.config_params.phase_lock_offset",
                                config.phase_lock_offset, true);

        const std::string signal_multiplier_string = "$.config_params.signal_multiplier";
        if (parse_and_validate_item(issues_.information, signal_multiplier_string,
                                    config.signal_multiplier, true)) {
            try {
                check_signal_multiplier(*config.signal_multiplier);
            } catch (std::runtime_error& e) {
                auto entry = ValidatorIssues::ValidatorEntry(signal_multiplier_string, e.what());
        ...
        parse_and_validate_item(issues_.information, "$.config_params.sync_pulse_out_angle",
                                config.sync_pulse_out_angle, make_verify_in_bounds<int>(0, 360),
                                true);

        parse_and_validate_item(issues_.information, "$.config_params.sync_pulse_out_frequency",
        ...
        parse_and_validate_item(issues_.information, "$.config_params.sync_pulse_out_pulse_width",
                                config.sync_pulse_out_pulse_width, true);

        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.timestamp_mode",
                                             config.timestamp_mode, timestamp_mode_of_string);

        if (!parse_and_validate_item(issues_.information, "$.config_params.udp_dest",
                                     config.udp_dest)) {
            parse_and_validate_item(issues_.information, "$.config_params.udp_ip", config.udp_dest);
        }

        parse_and_validate_item(issues_.information, "$.config_params.udp_dest_zm",
                                config.udp_dest_zm);

        parse_and_validate_item(issues_.information, "$.config_params.udp_port_imu",
                                config.udp_port_imu, make_verify_in_bounds<uint16_t>(0, 65535));

        parse_and_validate_item(issues_.information, "$.config_params.udp_port_lidar",
                                config.udp_port_lidar, make_verify_in_bounds<uint16_t>(0, 65535));

        parse_and_validate_item(issues_.information, "$.config_params.udp_port_zm",
                                config.udp_port_zm, make_verify_in_bounds<uint16_t>(0, 65535));

        parse_and_validate_item(issues_.information, "$.config_params.udp_multicast_ttl",
                                config.udp_multicast_ttl);

        parse_and_validate_item(issues_.information, "$.config_params.udp_multicast_ttl_zm",
                                config.udp_multicast_ttl_zm);

        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.udp_profile_imu",
        ...
        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.header_type",
                                             config.header_type, udp_profile_type_of_string);

        parse_and_validate_enum<std::string>(
            issues_.information, "$.config_params.bloom_reduction_optimization",
            config.bloom_reduction_optimization, bloom_reduction_optimization_of_string);

        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.gyro_fsr",
                                             config.gyro_fsr, full_scale_range_of_string);

        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.accel_fsr",
                                             config.accel_fsr, full_scale_range_of_string);

        parse_and_validate_enum<std::string>(issues_.information, "$.config_params.return_order",
                                             config.return_order, return_order_of_string);

        parse_and_validate_item(issues_.information, "$.config_params.min_range_threshold_cm",
                                config.min_range_threshold_cm);

        parse_and_validate_item(issues_.information, "$.config_params.imu_packets_per_frame",
                                config.imu_packets_per_frame);

        // parse all remaining items
        static const std::set<std::string> standard_set = {"accel_fsr",
                                                           "return_order",
    ...
    // lidar_mode
    bool internal_parse_and_validate_columns_per_frame(SensorInfo& sensor_info) {
        bool columns_per_frame_success = false;
        if (parse_and_validate_item<uint32_t>(issues_.information, columns_per_frame_string_,
                                              sensor_info.format.columns_per_frame)) {
            if (have_lidar_mode_) {
                // lidar mode is present, check that number of columns actually
                // matches config value
                auto temp_cols = sensor_info.config.lidar_mode->columns;
                if (sensor_info.format.columns_per_frame != temp_cols) {
                    // found a misconfiguration
                    std::stringstream error_message;
                    error_message << columns_per_frame_string_ << "("
                                  << sensor_info.format.columns_per_frame << ") does not match "
                                  << lidar_mode_string_ << "(" << temp_cols << ")";

                    auto entry = ValidatorIssues::ValidatorEntry(columns_per_frame_string_,
                                                                 error_message.str());
                    issues_.warning.push_back(entry);
                } else {
                ...
                // nothing to match
                skipped_due_to_item(issues_.information, columns_per_frame_string_,
                                    lidar_mode_string_,
            ...
            if (!have_lidar_mode_) {
                auto entry =
                    ValidatorIssues::ValidatorEntry(columns_per_frame_string_, "Missing field");
                issues_.critical.push_back(entry);
            }
    ...
    // prod_line
    void internal_parse_and_validate_pixels_per_column(SensorInfo& sensor_info) {
        if (parse_and_validate_item(issues_.information, pixels_per_column_string_,
            ...
            if (have_prod_line_) {
                ...
                if (sensor_info.format.pixels_per_column !=
                    ...
                    error_message << pixels_per_column_string_ << "("
                                  << sensor_info.format.columns_per_frame << ") does not match "
                                  << prod_line_string_ << "(" << temp_prod_line.beam_count
                                  << ") details";
                ...
                // nothing to match
                skipped_due_to_item(issues_.information, pixels_per_column_string_,
                                    prod_line_string_);
            }
        } else {
            if (have_prod_line_) {
                ...
                auto temp_prod_line = sensor_info.get_product_info();
                sensor_info.format.pixels_per_column = temp_prod_line.beam_count;
    ...
    void parse_and_validate_data_format(SensorInfo& sensor_info) {
        if (have_lidar_mode_) {
            ...
            sensor_info.format = default_data_format(*sensor_info.config.lidar_mode);
        }

        bool columns_per_frame_success = internal_parse_and_validate_columns_per_frame(sensor_info);

        const std::string column_window_string = "$.lidar_data_format.column_window.*";
        ...
        auto column_window_callback = [&](ValidatorIssues::EntryList& severity,
                                          const std::string& path, uint16_t data) {
            if (columns_per_frame_success) {
                return make_verify_in_bounds<uint16_t>(0, sensor_info.format.columns_per_frame - 1)(
                    severity, path, data);
            } else {
                return true;
            }
        };
        if (parse_and_validate_item(issues_.information, column_window_string, column_window_data,
                                    2, column_window_callback)) {
            sensor_info.format.column_window =
                std::make_pair(column_window_data[0], column_window_data[1]);
        }

        parse_and_validate_item(issues_.information, "$.lidar_data_format.columns_per_packet",
                                sensor_info.format.columns_per_packet);

        internal_parse_and_validate_pixels_per_column(sensor_info);

        const std::string pixel_shift_string = "$.lidar_data_format.pixel_shift_by_row.*";
        int verify_count = 0;
        if (have_prod_line_) {
            auto temp_prod_line = sensor_info.get_product_info();
            verify_count = temp_prod_line.beam_count;
        } else {
            skipped_due_to_item(issues_.information, pixel_shift_string + ".length()",
                                prod_line_string_,
                                "Product line not found, can't verify size of "
                                "pixel shift array");
        }
        parse_and_validate_item<int>(issues_.information, pixel_shift_string,
                                     sensor_info.format.pixel_shift_by_row, verify_count);
        // pad pixel shift by row with all zeros if it isnt present or isnt
        // large enough
        if (sensor_info.format.pixel_shift_by_row.size() != sensor_info.format.pixels_per_column) {
            sensor_info.format.pixel_shift_by_row.resize(sensor_info.format.pixels_per_column);
        }
        ...
        parse_and_validate_item(issues_.information, "$.imu_data_format.imu_packets_per_frame",
        ...
        if (!parse_and_validate_item(issues_.information, fps_string, sensor_info.format.fps)) {
            if (have_lidar_mode_) {
                sensor_info.format.fps = sensor_info.config.lidar_mode->fps;
                default_message(fps_string);
            } else {
                skipped_due_to_item(issues_.information, fps_string, lidar_mode_string_,
        ...
        if (sensor_info.config.udp_dest_zm) {
            ...
            if (!zm_dest.empty() && sensor_info.config.udp_port_zm.value_or(0) != 0) {
    ...
    void parse_and_validate_calibration_status(SensorInfo& sensor_info) {
        parse_and_validate_datetime(issues_.information,
                                    "$.calibration_status.reflectivity.timestamp", "%Y-%m-%dT%T",
                                    sensor_info.cal.reflectivity_timestamp);

        if (!parse_and_validate_item(issues_.information, "$.calibration_status.reflectivity.valid",
     ...
     */
    void parse_and_validate_angles(const std::string& path, std::vector<double>& output,
                                   size_t width, size_t height) {
        jsoncons::json value_array = impl::jsonpath_query(root_, path);
        jsoncons::json angles;
        if (value_array.empty()) {
            auto entry = ValidatorIssues::ValidatorEntry(path, "Missing intrinsics field.");
            issues_.critical.push_back(entry);
            return;
        }
        ...
        if (angles.is_null()) {
            // error
            auto entry = ValidatorIssues::ValidatorEntry(path, "Missing beam angle.");
            issues_.warning.push_back(entry);
            return;
        }
        ...
        for (const auto& val : angles.array_range()) {
            if (val.is_array()) {
                ...
                if (count != height) {
                    auto entry = ValidatorIssues::ValidatorEntry(
                        path, "Each beam angle sub-array must have " + std::to_string(height) +
                                  " elements.");
                    issues_.critical.push_back(entry);
                    return;
                }
        ...
        if (!is_doubles && !is_arrays) {
            // zero size
            auto entry = ValidatorIssues::ValidatorEntry(path, "Cannot be empty beam angle array.");
            issues_.critical.push_back(entry);
            return;
        }
        ...
        if (angles.size() != width) {
            auto entry = ValidatorIssues::ValidatorEntry(
                path, "Must have beam angle " + std::to_string(width) + " elements. Had " +
    ...
    void parse_and_validate_intrinsics(SensorInfo& sensor_info) {
        ...
        const std::string imu_intrinsics_string = "$.imu_intrinsics.imu_to_sensor_transform.*";
        if (parse_and_validate_item<double>(issues_.information, imu_intrinsics_string,
                                            imu_intrinsics_data, 16, true)) {
            decode_transform_array(sensor_info.imu_to_sensor_transform, imu_intrinsics_data);
        } else {
        ...
        if (parse_and_validate_item<double>(issues_.information, lidar_intrinsics_string,
                                            lidar_intrinsics_data, 16, true)) {
            decode_transform_array(sensor_info.lidar_to_sensor_transform, lidar_intrinsics_data);
        } else {
        ...
        // parse beam angles
        parse_and_validate_angles(
            "$.beam_intrinsics.beam_altitude_angles", sensor_info.beam_altitude_angles,
            sensor_info.format.pixels_per_column, sensor_info.format.columns_per_frame);
        parse_and_validate_angles(
            "$.beam_intrinsics.beam_azimuth_angles", sensor_info.beam_azimuth_angles,
            sensor_info.format.pixels_per_column, sensor_info.format.columns_per_frame);

        const std::string lidar_origin_string = "$.beam_intrinsics.lidar_origin_to_beam_origin_mm";
        if (!parse_and_validate_item(issues_.information, lidar_origin_string,
        ...
        const std::string beam_to_lidar_string = "$.beam_intrinsics.beam_to_lidar_transform.*";
        std::vector<double> beam_to_lidar_data;
        if (parse_and_validate_item<double>(issues_.information, beam_to_lidar_string,
                                            beam_to_lidar_data, 16, true)) {
            decode_transform_array(sensor_info.beam_to_lidar_transform, beam_to_lidar_data);
        } else {
            sensor_info.beam_to_lidar_transform = mat4d::Identity();
            sensor_info.beam_to_lidar_transform(0, 3) = sensor_info.lidar_origin_to_beam_origin_mm;
    ...
    void parse_and_validate_misc(SensorInfo& sensor_info) {
        ...
        if (parse_and_validate_item<double>(issues_.information, extrinsic_string, extrinsic_data,
                                            16, true)) {
            decode_transform_array(sensor_info.sensor_to_body, extrinsic_data);
        } else {
            default_message(extrinsic_string);
            sensor_info.sensor_to_body = mat4d::Identity();
        }

        parse_and_validate_item(issues_.information, "$.user_data", sensor_info.user_data);

        parse_and_validate_item(issues_.information, "$.'ouster-sdk'.client_version",
                                sensor_info.client_version);
...
class ConfigImpl : public SensorInfoImpl {
   public:
    ConfigImpl(const jsoncons::json& root, SensorConfig& config, ValidatorIssues& issues)
...
const std::map<std::string, bool> NONLEGACY_METADATA_FIELDS = {
    {"sensor_info", true},        {"beam_intrinsics", true}, {"imu_intrinsics", true},
    {"lidar_intrinsics", true},   {"config_params", true},   {"lidar_data_format", false},
...
bool is_new_format(const jsoncons::json& root) {
    ...
    if (nonlegacy_fields_present > 0 &&
        ...
        throw std::runtime_error{"Non-legacy metadata must include fields: " + missing_fields};
    }
...
jsoncons::json convert_legacy_to_nonlegacy(const jsoncons::json& root) {
    ...
    const std::vector<std::string> beam_intrinsics_fields{
        "lidar_origin_to_beam_origin_mm", "beam_altitude_angles", "beam_azimuth_angles",
    ...
    if (root.contains("imu_to_sensor_transform")) {
        result["imu_intrinsics"]["imu_to_sensor_transform"] = root.at("imu_to_sensor_transform");
...
bool parse_and_validate_metadata(const jsoncons::json& root, SensorInfo& sensor_info,
    ...
    for (const auto& field_pair : NONLEGACY_METADATA_FIELDS) {
        if (root.contains(field_pair.first)) {
            ...
            auto entry = ValidatorIssues::ValidatorEntry("$." + field_pair.first,
    ...
    if (nonlegacy_fields_present != NONLEGACY_METADATA_FIELDS.size()) {
        SensorInfoImpl impl(convert_legacy_to_nonlegacy(root), sensor_info, issues);
    } else {
    ...
    if ((!issues.information.empty()) || (!issues.warning.empty()) || (!issues.critical.empty())) {
...
bool parse_and_validate_metadata(const std::string& json_data, SensorInfo& sensor_info,
                                 ValidatorIssues& issues) {
...
bool parse_and_validate_config(const std::string& config, SensorConfig& sensor_config,
                               ValidatorIssues& issues) {
    jsoncons::json root;
    root["config_params"] = jsoncons::json::parse(config);

    ConfigImpl impl(root, sensor_config, issues);
...
bool parse_and_validate_config(const std::string& config, SensorConfig& sensor_config) {
    ValidatorIssues issues;
    return parse_and_validate_config(config, sensor_config, issues);
}
