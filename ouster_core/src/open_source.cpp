...
namespace impl {
std::map<core::IoType, FrameSetBuilderT>& get_builders() {
    static std::map<core::IoType, FrameSetBuilderT> builders;
    return builders;
}
...
std::string read_file(const std::string& file_name) {
    ...
    if (file_size <= 0) {
        throw std::runtime_error("Failed to read file or file is empty: " + file_name);
    }
...
namespace core {
void populate_extrinsics(const std::string& extrinsics_file,
    ...
    if (!extrinsics_file.empty()) {
        ...
        if (data.contains("transforms")) {
            for (const auto& obj : data["transforms"].array_range()) {
                ...
                auto rot =
                    Eigen::Quaternion<double>(qw, qx, qy, qz).normalized().toRotationMatrix();
    ...
    if (!extrinsics.empty()) {
        for (size_t i = 0; i < std::min(extrinsics.size(), sensor_infos.size()); i++) {
            sensor_infos[i]->sensor_to_body = extrinsics[i];
        }
...
std::vector<std::vector<FieldType>> resolve_field_types(
    const std::vector<std::shared_ptr<SensorInfo>>& sensor_infos, bool raw_headers, bool raw_fields,
    const nonstd::optional<std::vector<std::string>>& field_names) {
    std::vector<std::vector<FieldType>> all_ftypes;
    for (const auto& meta : sensor_infos) {
        ...
        if (raw_fields) {
            ...
            if (meta->format.udp_profile_lidar == UDPProfileLidar::RNG15_RFL8_NIR8_DUAL) {
                ftypes.emplace_back(ChanField::RAW32_WORD2, ChanFieldType::UINT32);
            } else if (meta->format.udp_profile_lidar != UDPProfileLidar::RNG15_RFL8_NIR8) {
                ftypes.emplace_back(ChanField::RAW32_WORD2, ChanFieldType::UINT32);
                ftypes.emplace_back(ChanField::RAW32_WORD3, ChanFieldType::UINT32);
                if (dual) {
                    ftypes.emplace_back(ChanField::RAW32_WORD4, ChanFieldType::UINT32);
                }
            }
        }
        ...
        if (raw_headers) {
            ...
            auto raw_headers_space =
                (packet_format.packet_header_size + packet_format.packet_footer_size +
        ...
        if (field_names) {
            ...
            for (const auto& name : field_names.value()) {
                ...
                if (!found) {
                    throw std::runtime_error("Requested field '" + name +
...
core::AnyFrameSetSource open_source(const std::string& source,
                                    const std::function<void(FrameSetSourceOptions&)>& options,
                                    bool collate, int sensor_idx) {
    std::vector<std::string> list = {source};
    return open_source(list, options, collate, sensor_idx);
}

core::AnyFrameSetSource open_source(const std::vector<std::string>& source,
                                    const std::function<void(FrameSetSourceOptions&)>& options,
                                    bool collate, int sensor_idx) {
    ...
    auto type = ouster::sdk::core::io_type(source[0]);
    for (size_t i = 1; i < source.size(); i++) {
        if (ouster::sdk::core::io_type(source[i]) != type) {
            throw std::runtime_error("All sources must have the same type.");
        }
    }
    if (impl::get_builders().find(type) != impl::get_builders().end()) {
        auto result = impl::get_builders()[type](source, opts, collate, sensor_idx);
        return core::AnyFrameSetSource(std::move(result));
    } else {
        if (type == IoType::OSF || type == IoType::PCAP || type == IoType::SENSOR) {
            throw std::runtime_error("Could not open frame set source. Unhandled source type " +
                                     to_string(type) +
...
core::AnyPacketSource open_packet_source(const std::string& source,
                                         const std::function<void(PacketSourceOptions&)>& options) {
    std::vector<std::string> list = {source};
    return open_packet_source(list, options);
}

core::AnyPacketSource open_packet_source(const std::vector<std::string>& source,
                                         const std::function<void(PacketSourceOptions&)>& options) {
    ...
    auto type = ouster::sdk::core::io_type(source[0]);
    if (impl::get_packet_builders().find(type) != impl::get_packet_builders().end()) {
        return core::AnyPacketSource(impl::get_packet_builders()[type](source, opts));
    } else {
        if (type == IoType::PCAP || type == IoType::SENSOR) {
            throw std::runtime_error("Could not open packet source. Unhandled source type " +
                                     to_string(type) +
                                     ". Perhaps you forgot to include the requisite source type or "
                                     "built the library without the correct build flags.");
        } else {
...
void PacketSourceOptions::check(const char* source_type) const {
    ...
    rcvbuf_size.check("rcvbuf_size", source_type);
}

void FrameSetSourceOptions::check(const char* source_type) const {
    extrinsics_file.check("extrinsics_file", source_type);
    extrinsics.check("extrinsics", source_type);
...
PacketSourceOptions::PacketSourceOptions(FrameSetSourceOptions& options)
    : extrinsics_file(options.extrinsics_file.move()),
      ...
      rcvbuf_size(options.rcvbuf_size.move()) {}
