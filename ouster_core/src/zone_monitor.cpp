...
#include "ouster/core/impl/threadpool.h"
...
const auto SCHEMA_JSON = jsoncons::json::parse(static_cast<const char*>(METADATA_SCHEMA));
const auto SCHEMA = jsoncons::jsonschema::make_schema(SCHEMA_JSON);
const auto VALIDATOR = jsoncons::jsonschema::json_validator<jsoncons::json>(SCHEMA);
...
class FileProvider {
   public:
    ...
    virtual std::vector<uint8_t> get_file(const std::string& filename) = 0;
    virtual std::string get_file_as_string(const std::string& filename) = 0;
};

class ZipFileProvider : public FileProvider {
    std::unique_ptr<Zip> zip_ptr_;

   public:
    explicit ZipFileProvider(std::unique_ptr<Zip> zip_ptr) : zip_ptr_(std::move(zip_ptr)) {}
    std::vector<uint8_t> get_file(const std::string& filename) override {
        return zip_ptr_->get_file(filename);
    }
    std::string get_file_as_string(const std::string& filename) override {
        return zip_ptr_->get_file_as_string(filename);
    }
};

class DirectoryFileProvider : public FileProvider {
    std::string dir_path_;

   public:
    explicit DirectoryFileProvider(std::string dir_path) : dir_path_(std::move(dir_path)) {}
    std::vector<uint8_t> get_file(const std::string& filename) override {
        return ouster::sdk::core::get_file_as_bytes(dir_path_ + "/" + filename);
    }
    std::string get_file_as_string(const std::string& filename) override {
        auto bytes = get_file(filename);
        return std::string(bytes.begin(), bytes.end());
    }
};

bool populate_zone_set(FileProvider& file_provider, ZoneSet& zone_set_config,
                       ValidatorIssues& issues) {
    jsoncons::json metadata =
        jsoncons::json::parse(file_provider.get_file_as_string("metadata.json"));
    ...
    if (metadata.contains("sensor_to_body_transform")) {
        std::array<double, 16> sensor_to_body_transform_data =
            metadata.at("sensor_to_body_transform").as<std::array<double, 16>>();
        zone_set_config.sensor_to_body_transform = mat4d_from_array(sensor_to_body_transform_data);
    }
    ...
    for (auto& zone_json_it : metadata.at("zones").object_range()) {
        ...
        if (!Zone::string_to_zonemode(zone_json.at("mode").as<std::string>(), zone_mode)) {
        ...
        if (zone_json.contains("stl")) {
            ...
            auto coordinate_frame_str = stl_json.at("coordinate_frame").as<std::string>();

            zone.stl = ouster::sdk::core::Stl(file_provider.get_file(file_name));
            zone.stl->filename = file_name;
            if (!ouster::sdk::core::Stl::string_to_coordinate_frame(coordinate_frame_str,
                                                                    zone.stl->coordinate_frame)) {
                std::stringstream err;
        ...
        if (zone_json.contains("zrb")) {
            ...
            zone.zrb = ouster::sdk::core::Zrb(file_provider.get_file(file_name));
        }
...
void ZoneSet::check_invariants() const {
    ...
    for (const auto& pair : zones) {
        ...
        try {
            ...
            std::stringstream err;
            err << "ZoneSet: Zone " << zone_id << " failed invariant check: " << e.what();
            throw std::logic_error(err.str());
        }

        if (!(zone.stl || zone.zrb)) {
            throw std::logic_error("ZoneSet: all Zones must have either an STL or ZRB file.");
        }
        ...
        if (zone.zrb) {
            if (first_zrb_n_rows == -1 && first_zrb_n_cols == -1) {
                ...
                if (zone.zrb->near_range_mm.rows() != first_zrb_n_rows ||
                    ...
                    throw std::logic_error("ZoneSet: all ZRBs must have the same resolution.");
                }
            }
        }
    }
...
ZoneSet::ZoneSet(const std::string& path) {  // NOLINT(cppcoreguidelines-pro-type-member-init)
    ValidatorIssues issues;
    auto is_dir = ouster::sdk::core::is_directory(path);
    if (is_dir && *is_dir) {
        DirectoryFileProvider file_provider(path);
        if (!populate_zone_set(file_provider, *this, issues)) {
            std::stringstream msg;
            msg << "Failed to create ZoneSet due to the following errors: " << std::endl
                << issues.to_string();

            throw std::invalid_argument(msg.str());
        }
    } else {
        auto zip_bytes = ouster::sdk::core::get_file_as_bytes(path);
        auto zip = std::make_unique<Zip>(zip_bytes);
        ZipFileProvider file_provider(std::move(zip));
        if (!populate_zone_set(file_provider, *this, issues)) {
            std::stringstream msg;
            msg << "Failed to create ZoneSet due to the following errors: " << std::endl
                << issues.to_string();
...
ZoneSet::ZoneSet(
    const std::vector<uint8_t>& zip_bytes) {  // NOLINT(cppcoreguidelines-pro-type-member-init)
    ValidatorIssues issues;
    auto zip = std::make_unique<Zip>(zip_bytes);
    ZipFileProvider file_provider(std::move(zip));
    if (!populate_zone_set(file_provider, *this, issues)) {
        ...
        items[future_idx++] = pool.enqueue([&]() {
            ...
            if (zone.zrb && !zone.stl) {
                ...
                if (zone.render(beam_config)) {
                    ...
                    throw std::runtime_error("ZoneSet::render: zone " + std::to_string(iter.first) +
...
std::vector<uint8_t> ZoneSet::to_zip_blob(ZoneSetOutputFilter zone_set_output_filter) const {
    std::string metadata = to_json(zone_set_output_filter);  // calls check_invariants()
    ...
    for (const auto& pair : zones) {
        ...
        if (stl && zone_set_output_filter != ZoneSetOutputFilter::ZRB) {
            auto file_name = stl->filename.empty() ? zone_id + ".stl" : stl->filename;
            std::vector<uint8_t> blob = stl->blob();
            if (blob.empty()) {
                throw std::runtime_error("Zone " + zone_id + " has an empty stl blob");
            }
        ...
        if (zrb && zone_set_output_filter != ZoneSetOutputFilter::STL) {
            ...
            if (blob.empty()) {
                throw std::runtime_error("Zone " + zone_id + " has an empty zrb blob");
            }
            zip.add_file(file_name, blob);
        }
    }
...
void ZoneSet::save(const std::string& zip_path, ZoneSetOutputFilter zone_set_output_filter) const {
    std::vector<uint8_t> zip = to_zip_blob(zone_set_output_filter);  // calls check_invariants()

    std::ofstream file(zip_path, std::ios::binary | std::ios::out);
    file.write(reinterpret_cast<char*>(zip.data()), static_cast<std::streamsize>(zip.size()));
}

void ZoneSet::save_to_directory(const std::string& dir_path,
                                ZoneSetOutputFilter zone_set_output_filter) const {
    std::string metadata = to_json(zone_set_output_filter);  // calls check_invariants()
    VALIDATOR.validate(jsoncons::json::parse(metadata));

    if (!ouster::sdk::core::make_dir(dir_path)) {
        throw std::runtime_error("Failed to create directory: " + dir_path);
    }

    std::ofstream metadata_file(dir_path + "/metadata.json");
    ...
    for (const auto& pair : zones) {
        ...
        if (stl && zone_set_output_filter != ZoneSetOutputFilter::ZRB) {
            ...
            std::ofstream stl_file(dir_path + "/" + file_name, std::ios::binary | std::ios::out);
            stl_file.write(reinterpret_cast<const char*>(blob.data()),
        ...
        if (zrb && zone_set_output_filter != ZoneSetOutputFilter::STL) {
            ...
            // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
            std::ofstream zrb_file(dir_path + "/" + file_name, std::ios::binary | std::ios::out);
            zrb_file.write(reinterpret_cast<const char*>(blob.data()),
                           static_cast<std::streamsize>(blob.size()));
        }
    }
}
...
std::string ZoneSet::to_json(ZoneSetOutputFilter zone_set_output_filter) const {
    ...
    for (const auto& pair : zones) {
        ...
        auto& zrb = zone.zrb;
        if (stl && zone_set_output_filter != ZoneSetOutputFilter::ZRB) {
            auto file_name = stl->filename.empty() ? id + ".stl" : stl->filename;
            result["zones"][id]["stl"]["file_name"] = file_name;
            result["zones"][id]["stl"]["coordinate_frame"] = to_string(stl->coordinate_frame);
    ...
    if (sensor_to_body_transform) {
        result["sensor_to_body_transform"] = mat4d_to_array(*sensor_to_body_transform);
    }
...
bool operator==(const ZoneSet& lhs, const ZoneSet& rhs) {
    ...
    return lhs.zones == rhs.zones && lhs.power_on_live_ids == rhs.power_on_live_ids &&
