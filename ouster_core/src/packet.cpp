...
PacketValidationFailure Packet::validate(const SensorInfo& info) const {
    return ouster::sdk::core::validate_packet(info, *format, buf.data(), buf.size(), type_);
}

PacketValidationFailure Packet::validate(const SensorInfo& info, const PacketFormat& format) const {
    return ouster::sdk::core::validate_packet(info, format, buf.data(), buf.size(), type_);
}

PacketValidationFailure validate_packet(const SensorInfo& info, const PacketFormat& format,
    ...
    if (type == PacketType::Imu && format.udp_profile_imu == UDPProfileIMU::LEGACY) {
...
LidarPacket::LidarPacket(std::shared_ptr<PacketFormat> format)
    : Packet{MY_TYPE, static_cast<int>(format->lidar_packet_size)} {
...
ImuPacket::ImuPacket(std::shared_ptr<PacketFormat> format)
    : Packet{MY_TYPE, static_cast<int>(format->imu_packet_size)} {
    this->format = std::move(format);
}
...
ZonePacket::ZonePacket(std::shared_ptr<PacketFormat> format)
    : Packet{MY_TYPE, static_cast<int>(format->zone_packet_size)} {
    this->format = std::move(format);
}
...
Eigen::Array<float, Eigen::Dynamic, 3> ImuPacket::accel() const {
    ...
    for (size_t i = 0; i < measurements_per_packet; ++i) {
        ...
        out(static_cast<Eigen::Index>(i), 0) = format->imu_la_x(ptr);
        out(static_cast<Eigen::Index>(i), 1) = format->imu_la_y(ptr);
        out(static_cast<Eigen::Index>(i), 2) = format->imu_la_z(ptr);
    }
    return out;
}
...
Eigen::Array<float, Eigen::Dynamic, 3> ImuPacket::gyro() const {
    ...
    for (size_t i = 0; i < measurements_per_packet; ++i) {
        const uint8_t* ptr = format->imu_nth_measurement(i, buf.data());
        out(static_cast<Eigen::Index>(i), 0) = format->imu_av_x(ptr);
        out(static_cast<Eigen::Index>(i), 1) = format->imu_av_y(ptr);
        out(static_cast<Eigen::Index>(i), 2) = format->imu_av_z(ptr);
    }
    return out;
}
...
Eigen::Array<uint16_t, Eigen::Dynamic, 1> ImuPacket::status() const {
    ...
    for (size_t i = 0; i < measurements_per_packet; ++i) {
        const uint8_t* ptr = format->imu_nth_measurement(i, buf.data());
        out(static_cast<Eigen::Index>(i), 0) = format->col_status(ptr) & 0x1;
    }
    return out;
}
...
Eigen::Array<uint64_t, Eigen::Dynamic, 1> ImuPacket::timestamp() const {
    if (format->udp_profile_imu == UDPProfileIMU::LEGACY) {
        ...
        out(0, 0) = format->imu_gyro_ts(buf.data());
        return out;
    }
    ...
    for (size_t i = 0; i < measurements_per_packet; ++i) {
        const uint8_t* ptr = format->imu_nth_measurement(i, buf.data());
        out(static_cast<Eigen::Index>(i), 0) = format->col_timestamp(ptr);
    }
    return out;
}
...
Eigen::Array<uint16_t, Eigen::Dynamic, 1> ImuPacket::measurement_id() const {
    ...
    for (size_t i = 0; i < measurements_per_packet; ++i) {
        const uint8_t* ptr = format->imu_nth_measurement(i, buf.data());
        out(static_cast<Eigen::Index>(i), 0) = format->col_measurement_id(ptr);
    }
    return out;
}
...
Eigen::Array<ZoneState, Eigen::Dynamic, 1> ZonePacket::zone_states() const {
    ...
    for (size_t i = 0; i < 16; ++i) {
        ...
        auto& zone = out(static_cast<Eigen::Index>(i), 0);
        zone.live = static_cast<uint8_t>(format->zone_live(zone_ptr));
        zone.id = format->zone_id(zone_ptr);
        zone.error_flags = format->zone_error_flags(zone_ptr);
        zone.trigger_type = format->zone_trigger_type(zone_ptr);
        zone.trigger_status = format->zone_trigger_status(zone_ptr);
        zone.triggered_frames = format->zone_triggered_frames(zone_ptr);
        zone.count = format->zone_points_count(zone_ptr);
        zone.occlusion_count = format->zone_occlusion_count(zone_ptr);
        zone.invalid_count = format->zone_invalid_count(zone_ptr);
        zone.max_count = format->zone_max_count(zone_ptr);
        zone.min_range = format->zone_min_range(zone_ptr);
        zone.max_range = format->zone_max_range(zone_ptr);
        zone.mean_range = format->zone_mean_range(zone_ptr);
    }
    return out;
}
}  // namespace core
