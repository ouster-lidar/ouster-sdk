...
class VectorStreamBuf : public std::streambuf {
   public:
    using ByteVector = std::vector<uint8_t>;  ///< An alias for uint8_t vector type
   ...
   protected:
     ...
     * @return The new position, or -1 on error.
     */
    std::streampos seekpos(std::streampos position, std::ios_base::openmode openmode) override;
