...
class Logger {
   private:
    ...
    std::string finalize_format_builder(const std::shared_ptr<Logger::format_builder>& builder);
   ...
   public:
    ...
    enum LOG_LEVEL { LOG_TRACE, LOG_DEBUG, LOG_INFO, LOG_WARN, LOG_ERROR, LOG_CRITICAL };
    ...
    bool configure_file_sink(const std::string& log_level, const std::string& log_file_path,
    ...
    void log(LOG_LEVEL level, const std::string& format_string, Args&&... args) {
        std::shared_ptr<Logger::format_builder> builder = make_builder(format_string);
   ...
   private:
    // Base Case
    template <typename T>
    void process_args(const std::shared_ptr<Logger::format_builder>& builder, T next_arg) {
    ...
    template <typename T>
    void process_arg(const std::shared_ptr<Logger::format_builder>& builder, T data);
