...
// Non-template bridge functions so that jsoncons jsonpath templates are
// instantiated only once (in json_tools.cpp) instead of in every including TU.
jsoncons::json jsonpath_query(const jsoncons::json& root, const std::string& path);
void jsonpath_query(const jsoncons::json& root, const std::string& path,
                    std::function<void(const std::string&, const jsoncons::json&)> callback);
...
inline std::string fix_typename(const char* name) {
    ...
    std::unique_ptr<char, decltype(&std::free)> pointer(
        __cxxabiv1::__cxa_demangle(name, nullptr, &length, &status), &std::free);
    return pointer.get();
}
#else
/**
...
class JsonTools {
   protected:
    ...
    static void skipped_due_to_item(ValidatorIssues::EntryList& severity,
                                    const std::string& item_skipped, const std::string& cause_item,
     ...
     */
    static void decode_transform_array(mat4d& output, const std::vector<double>& data);
    ...
    static bool verify_all_not_zero(ValidatorIssues::EntryList& severity, const std::string& path,
                                    T& data) {
        uint64_t zeros = 0;
        for (auto it : data) {
            if (it == 0) {
        ...
        if (zeros == data.size()) {
            ...
            error_message << "Expected at least some non-zero values in metadata array";

            auto entry = ValidatorIssues::ValidatorEntry(path, error_message.str());
     ...
     */
    template <typename T, typename = std::enable_if<std::is_arithmetic<T>::value, T>>
    static std::function<bool(ValidatorIssues::EntryList&, const std::string&, T)>
    make_verify_in_bounds(T lower, T upper) {
        return
            [lower, upper](ValidatorIssues::EntryList& severity, const std::string& path, T data) {
                bool result = true;
                if (data < lower) {
                    std::stringstream error_message;
                    error_message << "Item value " << data << " is lower than the lower bound "
                                  << lower;
                ...
                if (data > upper) {
                    std::stringstream error_message;
                    error_message << "Item value " << data << " is greater than the upper bound "
                                  << upper;
     ...
     */
    template <typename T, typename F, typename = std::enable_if<std::is_function<F>::value, F>>
    bool parse_and_validate_item(ValidatorIssues::EntryList& severity, const std::string& path,
                                 T& output, F verification_callback,
        ...
        if (value_array.size() == 1) {
            ...
            if (value.is<T>() || (relaxed_number_verification && value.is_number() &&
                                  std::is_arithmetic<T>::value)) {
                // Warning: ran into a really weird argument swapping issue here
                // we think it was related to ABI issues.
                output = value.as<T>();
                bool temp_result = verification_callback(severity, path, value.as<T>());
                return temp_result;
            } else {
                try {
                    output = value.as<T>();
                ...
                std::stringstream error_message;
                error_message << "Type Expected: \"" << impl::fix_typename(typeid(T).name())
                              << "\" Actual Type: " << value.type() << "\" Value: \"" << value
                              << "\"";
                auto entry = ValidatorIssues::ValidatorEntry(path, error_message.str());
                severity.push_back(entry);
            }
            ...
            error_message << "Expected One Item In Data, "
                          << "Number Of Items: " << value_array.size() << " Values: \""
                          << value_array << "\"";
            auto entry = ValidatorIssues::ValidatorEntry(path, error_message.str());
            severity.push_back(entry);
        }
    ...
    template <typename T>
    bool parse_and_validate_item(ValidatorIssues::EntryList& severity, const std::string& path,
                                 T& output, bool relaxed_number_verification = false) {
        return parse_and_validate_item<T>(
            severity, path, output,
            [&](ValidatorIssues::EntryList& /*severity*/, const std::string& /*path*/, T /*data*/) {
                return true;
            },
    ...
    bool parse_and_validate_item(ValidatorIssues::EntryList& severity, const std::string& path,
                                 ...
                                 bool relaxed_number_verification = false) {
        T data;
        if (parse_and_validate_item<T>(severity, path, data, relaxed_number_verification)) {
            output = data;
            return true;
        } else {
            return false;
    ...
    template <typename T, typename F, typename = std::enable_if<std::is_function<F>::value, F>>
    bool parse_and_validate_item(ValidatorIssues::EntryList& severity, const std::string& path,
                                 nonstd::optional<T>& output, F verification_callback,
                                 bool relaxed_number_verification = false) {
        T data;
        bool temp_result = parse_and_validate_item<T, F>(
            severity, path, data, verification_callback, relaxed_number_verification);
        if (temp_result) {
            output = data;
        }
    ...
    bool parse_and_validate_item(ValidatorIssues::EntryList& severity, const std::string& path,
        ...
        auto parse_callback = [&](const std::string& path, const jsoncons::json& /*val*/) {
            T data;
            if (parse_and_validate_item<T>(severity, path, data, relaxed_number_verification)) {
                matches++;
            }
        ...
        jsonpath_query(root_, path, parse_callback);
        bool result = (index == matches && matches > 0);
        if (verify_count > 0 && matches != verify_count) {
            std::stringstream error_message;
            error_message << "Invalid metadata array, got " << index << " items, " << matches
                          << " matching items,"
                          << " was expecting " << verify_count << " matching items";
    ...
    bool parse_and_validate_item(ValidatorIssues::EntryList& severity, const std::string& path,
        ...
        auto parse_callback = [&](const std::string& path, const jsoncons::json& /*val*/) {
            T data;
            if (parse_and_validate_item<T, F>(severity, path, data, verification_callback,
                                              relaxed_number_verification)) {
                matches++;
            }
            shadow_output.push_back(data);
    ...
    bool parse_and_validate_enum(ValidatorIssues::EntryList& severity, const std::string& path,
        ...
        if (parse_and_validate_item<T>(severity, path, data)) {
            try {
                ...
                std::stringstream error_message;
                error_message << "Failed To Parse Enum: " << data << " Error Message: \""
                              << e.what() << "\"";
                severity.emplace_back(path, error_message.str());
                return false;
            }
    ...
    bool parse_and_validate_enum(ValidatorIssues::EntryList& severity, const std::string& path,
        ...
        bool result = parse_and_validate_enum<T, U, F>(severity, path, temp_output, func);
        if (result) {
            output = *temp_output;
        }
        return result;
     ...
     */
    template <typename T>
    bool parse_and_validate_datetime(ValidatorIssues::EntryList& severity, const std::string& path,
                                     const std::string& date_format, nonstd::optional<T>& output) {
        T data;
        if (parse_and_validate_item(severity, path, data, verify_string_not_empty)) {
            ...
            if (date_data.fail()) {
                std::stringstream error_message;
                error_message << "Build date not a properly formatted DateTime: \"";
                error_message << data << "\"";
    ...
    template <typename T>
    bool parse_and_validate_datetime(ValidatorIssues::EntryList& severity, const std::string& path,
                                     const std::string& date_format, T& output) {
        nonstd::optional<T> data;
        auto result = parse_and_validate_datetime(severity, path, date_format, data);
        if (data.has_value()) {
            output = *data;
        }
