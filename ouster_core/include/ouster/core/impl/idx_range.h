...
struct count_n_ranges {
    static constexpr int VALUE = count_n_ranges<T>::VALUE + count_n_ranges<Args...>::VALUE;
};
...
template <typename T>
OSDK_FN constexpr int is_idx_range(T /*unused*/) {
...
OSDK_FN std::enable_if_t<!is_idx_range_t<U>::V, T> range_replace_dim(T dim, U /*unused*/) {
    return dim;
}
...
OSDK_FN T range_replace_dim(T dim, idx_range e) {
    if (e.end == 0) {
        return dim;  // default empty idx_range
    }
 ...
 */
template <typename IntT, int N, int M, typename... Args>
OSDK_FN void range_args_restride(const IntT (&strides)[N], IntT (&new_strides)[M],
                                 Args... /*unused*/) {
    static_assert(M == N - sizeof...(Args) + count_n_ranges<Args...>::VALUE, "shape mismatch");
 ...
 */
template <typename IntT, int N, int M, typename... Args>
OSDK_FN void range_args_reshape(const IntT (&shape)[N], IntT (&new_shape)[M], Args... args) {
    int i = 0;
    IntT ranged_shape[N] = {range_replace_dim(shape[i++], args)...};
    for (; i < N; ++i) {
 ...
 */
template <typename T, typename... Args>
OSDK_FN_HOST std::vector<T> range_args_restride(const std::vector<T>& strides, Args... /*unused*/) {
    std::vector<T> out;
 ...
 */
template <typename T, typename... Args>
OSDK_FN_HOST std::vector<T> range_args_reshape(const std::vector<T>& shape, Args... args) {
    int i = 0;
...
OSDK_FN
OUSTER_API_FUNCTION
inline impl::idx_range keep() {
    return {0, 0};
}
OSDK_FN
OUSTER_API_FUNCTION
inline impl::idx_range keep(int start, int end) {
    return {start, end};
}
