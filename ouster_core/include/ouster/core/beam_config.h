...
struct OUSTER_API_CLASS BeamConfig {
    ...
    mat4d lidar_to_sensor_transform;                     ///< Transformation matrix from LIDAR to
                                                         ///< sensor frame
    nonstd::optional<mat4d> sensor_to_body_transform{};  ///< Extrinsic transformation matrix
    ...
    std::shared_ptr<XYZLut> lut_no_sensor_to_body_transform;  ///< Lookup table for converting
     ...
     */
    OUSTER_API_FUNCTION
    BeamConfig(uint32_t n_cols_init, const std::vector<double>& px_altitudes_init,
               const std::vector<double>& px_azimuths_init, mat4d beam_to_lidar_transform_init,
               mat4d lidar_to_sensor_transform_init,
               nonstd::optional<mat4d> sensor_to_body_transform_init, float m_per_zmbin_init,
