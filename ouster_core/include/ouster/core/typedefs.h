...
/**
 * @brief NxM array with row-major storage.
 * @tparam T The data type for the array.
 */
template <typename T>
...
using mat4d = Eigen::Matrix<double, 4, 4, Eigen::RowMajor | Eigen::DontAlign>;
...
/**
 * @brief NxM array with row-major storage (double)
 */
using ArrayXXdR = ArrayXXR<double>;

/**
 * @brief NxM array with row-major storage (float)
 */
...
struct OUSTER_API_CLASS float16_t {
     ...
     */
    OUSTER_API_FUNCTION explicit float16_t(uint16_t i) {
        data = i;
    }
};
#pragma pack(pop)
