...
class OUSTER_API_CLASS Mesh {
   public:
    ...
    /**
     * Writes the Triangles in this Mesh to an STL file in binary format.
     * @param[in] output_stream An ostream (perhaps an ofstream) to write the
     * STL ascii-formatted content to.
     */
    OUSTER_API_FUNCTION
    void save_stl_binary(std::ostream& output_stream) const;

    /**
     * Writes the Triangles in this Mesh to an STL file in binary format.
     * @param[in] path The path to the STL file to write to.
     */
    OUSTER_API_FUNCTION
    void save_stl_binary(const std::string& path) const;
