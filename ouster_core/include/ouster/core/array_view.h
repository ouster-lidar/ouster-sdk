...
#include "ouster/core/impl/cuda_macros.h"
...
struct product {
    ...
    T operator()(IterT iter, row_pack<I...> /*unused*/) {
        IterT i = --iter;
        return static_cast<T>(*i) * (*this)(i, typename drop_left<I...>::type{});
    }
...
class ArrayView {
   ...
   public:
    ...
    ArrayView(T* ptr, const ContainerT& shape)
        : ArrayView(ptr, std::begin(shape), std::end(shape), impl::index_row<Dim>{},
    ...
    ArrayView(T* ptr, const int (&shape)[Dim])
        : ArrayView(ptr, shape, shape + Dim, impl::index_row<Dim>{}, impl::index_row<Dim - 1>{}) {}
    ...
    template <typename OtherT,
              typename = std::enable_if_t<std::is_same<T, const OtherT>::VALUE, void>>
    ...
    OSDK_FN T& operator()(Args&&... idx) {
        return const_cast<T&>(const_cast<const ArrayView&>(*this)(std::forward<Args>(idx)...));
    }
    ...
    OSDK_FN ArrayView<T, subview_dim<Args...>()> subview(Args... idx) const {
        ...
        T* ptr = data_ + impl::strided_index(strides, impl::range_or_idx(idx)...);
    ...
    ArrayView<T, sizeof...(Args)> reshape(Args... dims) const {
        if (sparse()) {
            throw std::invalid_argument("ArrayView: cannot reshape sparse views");
        }

        if (impl::product<uint32_t>{}(dims...) != shape[0] * strides[0]) {
            throw std::invalid_argument("ArrayView: cannot reshape due to size mismatch");
        }

        return ArrayView<T, sizeof...(Args)>(data_, {static_cast<uint32_t>(dims)...});
    }
    ...
    OSDK_FN bool sparse() const {
        ...
        for (size_t i = 1; i < Dim; ++i) {
            dense = dense && (static_cast<uint32_t>(strides[i - 1]) == strides[i] * shape[i]);
        }
        return !dense;
    }
   ...
   private:
    ...
    ArrayView(T* ptr, IterT first, IterT last, impl::row_pack<I...> /*unused*/,
        ...
        : data_(ptr),
          strides{impl::product<int32_t>{}(last, impl::index_row<Dim - 1 - J>{})..., 1},
    ...
    OSDK_FN ArrayView(T* ptr, IntT* first, IntT* last, impl::row_pack<I...> /*unused*/,
                      impl::row_pack<J...> /*unused*/)
        : data_(ptr),
          strides{impl::product<int32_t>{}(last, impl::index_row<Dim - 1 - J>{})..., 1},
          shape{(void(I), static_cast<uint32_t>(*first++))...} {
     ...
     */
    template <int... I>
    OSDK_FN ArrayView(T* ptr, const int32_t (&shape)[Dim], const int32_t (&strides)[Dim],
                      impl::row_pack<I...> /*unused*/)
