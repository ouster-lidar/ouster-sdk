...
struct OUSTER_API_CLASS FrameSetSourceOptions {
    ...
    /// list of fields to decode into LidarFrames, if not set decodes all
    /// fields, if an empty array decodes no fields
    ...
    /// If true, accept packets/frames that don't match the init_id/sn of the
    /// metadata
    ...
    /// only applies to iterators, not get_frame
    ...
    /// maximum number of frames to queue, useful for limiting buffer bloat
...
OUSTER_API_FUNCTION
ouster::sdk::core::AnyFrameSetSource open_source(
    const std::string& source,                                        ///< [in] source filename(s)
    const std::function<void(FrameSetSourceOptions&)>& options = {},  ///< [in] source options
...
ouster::sdk::core::AnyFrameSetSource open_source(
    ...
    const std::function<void(FrameSetSourceOptions&)>& options = {}, bool collate = true,
    int sensor_idx = -1);
...
struct OUSTER_API_CLASS PacketSourceOptions {
    ...
    OUSTER_API_FUNCTION
    explicit PacketSourceOptions(FrameSetSourceOptions& options  ///< [in] options
    );
...
ouster::sdk::core::AnyPacketSource open_packet_source(
    ...
    const std::function<void(PacketSourceOptions&)>& options = {}  ///< [in] source options
);
...
void populate_extrinsics(
    const std::string& extrinsics_file,  ///< [in] optional extrinsics file name
...
std::vector<std::vector<FieldType>> resolve_field_types(
    const std::vector<std::shared_ptr<SensorInfo>>& sensor_infos,  ///< [in] SensorInfos
...
OUSTER_DEPRECATED_TYPE(ScanSourceOptions, FrameSetSourceOptions,
