...
struct OUSTER_API_CLASS ZoneState {
    ...
    uint32_t occlusion_count{0};   ///< Count of points below minimum range in the zone.
    uint32_t invalid_count{0};     ///< Count of invalid points in the zone.
    uint32_t max_count{0};         ///< Maximum count of points in the zone.

    uint32_t min_range{0};   ///< Minimum range of points in the zone in millimeters.
    uint32_t max_range{0};   ///< Maximum range of points in the zone in millimeters.
    uint32_t mean_range{0};  ///< Mean of points in the zone in millimeters.
};

/**
 * @brief Equality operator for ZoneState.
 *
 * Compares all fields of two ZoneState instances for equality.
 *
 * @param[in] lhs The left-hand side ZoneState instance.
 * @param[in] rhs The right-hand side ZoneState instance.
 *
 * @return true if all fields of lhs and rhs are equal, false otherwise.
 */
OUSTER_API_FUNCTION
bool operator==(const ZoneState& lhs, const ZoneState& rhs);

/**
 * @brief Inequality operator for ZoneState.
 *
 * Compares all fields of two ZoneState instances for inequality.
 *
 * @param[in] lhs The left-hand side ZoneState instance.
 * @param[in] rhs The right-hand side ZoneState instance.
 *
 * @return true if any field of lhs and rhs are not equal, false otherwise.
 */
OUSTER_API_FUNCTION
bool operator!=(const ZoneState& lhs, const ZoneState& rhs);
