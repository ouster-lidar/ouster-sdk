...
struct OUSTER_API_CLASS Packet {
   ...
   public:
    ...
    OUSTER_API_FUNCTION PacketType type() const {
        return type_;
    }
    ...
    OUSTER_API_FUNCTION
    PacketValidationFailure validate(const SensorInfo& info  ///< [in] expected sensor_metadata
    ) const;
   ...
   protected:
    ...
    inline void header_check() const {
        if (type_ == PacketType::Imu &&
            format->udp_profile_imu == ouster::sdk::core::UDPProfileIMU::LEGACY) {
...
struct OUSTER_API_CLASS LidarPacket : public Packet {
    ...
    OUSTER_DEPRECATED_MSG(LidarPacket(std::shared_ptr<PacketFormat>),
    ...
    OUSTER_DEPRECATED_MSG(LidarPacket(std::shared_ptr<PacketFormat>),
                          OUSTER_DEPRECATED_LAST_SUPPORTED_1_0)
    OUSTER_API_FUNCTION LidarPacket(int size  ///< [in] size in bytes to allocate
    );

    /**
     * Construct a new Lidar packet with a known format
     * @param[in] format the packet format to use for parsing this packet.
     */
    OUSTER_API_FUNCTION LidarPacket(std::shared_ptr<PacketFormat> format);
    ...
    OUSTER_API_FUNCTION inline auto nth_px(int n,                  ///< [in] which pixel
    ...
    void block_field(Eigen::Ref<img_t<T>> field,  ///< [out] destination eigen array
...
struct OUSTER_API_CLASS ImuPacket : public Packet {
    ...
    OUSTER_DEPRECATED_MSG(ImuPacket(std::shared_ptr<PacketFormat>),
                          OUSTER_DEPRECATED_LAST_SUPPORTED_1_0)
     ...
     * @param[in] format the packet format to use for parsing this packet.
     */
    OUSTER_API_FUNCTION ImuPacket(std::shared_ptr<PacketFormat> format);
    ...
    OUSTER_API_FUNCTION
    uint64_t nmea_ts() const {
        return format->imu_nmea_ts(buf.data());
    }
    ...
    OUSTER_API_FUNCTION Eigen::Array<uint16_t, Eigen::Dynamic, 1> status() const;

    /// Get read timestamps from imu packet.
    /// For UDPProfileIMU::LEGACY a (1,1) array representing gyro_ts is
    /// returned.
    /// @return 1d array of timestamps
    OUSTER_API_FUNCTION Eigen::Array<uint64_t, Eigen::Dynamic, 1> timestamp() const;
    ...
    OUSTER_API_FUNCTION Eigen::Array<uint16_t, Eigen::Dynamic, 1> measurement_id() const;
};
...
struct OUSTER_API_CLASS ZonePacket : public Packet {
    ...
    OUSTER_DEPRECATED_MSG(ZonePacket(std::shared_ptr<PacketFormat>),
                          OUSTER_DEPRECATED_LAST_SUPPORTED_1_0)
     ...
     * @param[in] format the packet format to use for parsing this packet.
     */
    OUSTER_API_FUNCTION ZonePacket(std::shared_ptr<PacketFormat> format);
    ...
    Eigen::Array<ZoneState, Eigen::Dynamic, 1> OUSTER_API_FUNCTION zone_states() const;
};
