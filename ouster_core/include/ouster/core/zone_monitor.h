...
struct OUSTER_API_CLASS ZoneSet {
    ...
    nonstd::optional<mat4d> sensor_to_body_transform{};  ///< sensor to body transform
    ...
    /**
     * Serializes this ZoneSet to a directory.
     *
     * @param[in] dir_path the directory path to write the files to.
     * @param[in] zone_set_output_filter specifies which files to include in the
     * resulting directory.
     *
     * @throws std::runtime_error on failing to produce files or missing
     *         critical data.
     */
    OUSTER_API_FUNCTION
    void save_to_directory(const std::string& dir_path,
     ...
     */
    OUSTER_API_FUNCTION std::string to_json(ZoneSetOutputFilter zone_set_output_filter) const;
     ...
     */
    friend OUSTER_API_FUNCTION bool operator==(const ZoneSet& lhs, const ZoneSet& rhs);
