...
class OUSTER_API_CLASS Zrb {
   public:
     ...
     */
    OUSTER_API_FUNCTION
    Zrb(uint32_t n_rows, uint32_t n_cols, float m_per_zmbin, uint64_t serial_number,
        Sha256 stl_hash, mat4d beam_to_lidar, mat4d lidar_to_sensor, mat4d sensor_to_body);
    ...
    mat4d lidar_to_sensor_transform{mat4d::Zero()};  ///< Lidar to sensor transform used
                                                     ///< during rendering
    mat4d sensor_to_body_transform{mat4d::Zero()};   ///< Sensor to body transform used during
                                                     ///< rendering
    uint64_t serial_number{0};                       ///< Serial number of the sensor that
    std::bitset<2048> valid_col_mask;                ///< Mask indicating which columns are valid
     ...
     */
    friend OUSTER_API_FUNCTION bool operator==(const Zrb& lhs, const Zrb& rhs);
