...
static constexpr cf_type GROUND = "GROUND";        ///< 1st return ground mask
static constexpr cf_type GROUND2 = "GROUND2";      ///< 2nd return ground mask
...
static constexpr cf_type ZONE_MASK = "ZONE_MASK";  ///< per pixel mask describing zone occupancy
...
static constexpr cf_type IMU_GYRO = "IMU_GYRO";  ///< IMU gyroscope read in degrees/second
...
static constexpr cf_type IMU_STATUS = "IMU_STATUS";  ///< status of the imu measurement
static constexpr cf_type IMU_PACKET_TIMESTAMP = "IMU_PACKET_TIMESTAMP";  ///< imu packet timestamps
static constexpr cf_type IMU_ALERT_FLAGS = "IMU_ALERT_FLAGS";            ///< imu alert flags
...
static constexpr cf_type LIVE_ZONESET_HASH =
    "LIVE_ZONESET_HASH";  ///< cryptographic hash of all zone configs and output
                          ...
                          ///< sensor
static constexpr cf_type ZONE_TIMESTAMP = "ZONE_TIMESTAMP";  ///< zone monitoring timestamp
...
static constexpr cf_type ZONE_ALERT_FLAGS = "ZONE_ALERT_FLAGS";  ///< zone alert flags

/// Build the field name for a given base field and 0-based return index.
/// @param[in] base  The base field name (e.g. "RANGE", "GROUND").
/// @param[in] ret   Sensor return index.
/// @return      base when ret==0, base+"2" when ret==1, base+"3" when ret==2.
inline OUSTER_API_FUNCTION std::string return_field_name(const std::string& base, int ret) {
    if (ret == 0) return base;
    return base + std::to_string(ret + 1);
}
...
/*
 * Value types for a Field.
 * THESE ENUM VALUES ARE SAVED DIRECTLY TO OSF AND MUST NOT CHANGE
 */
