...
bool set_pixel_from_viewport_coordinates(const std::shared_ptr<viz::Image>& img, viz::PointViz& viz,
                                         const viz::WindowCtx& ctx, double x, double y) {
...
bool mouse_button_handler(const std::shared_ptr<viz::Image>& img, viz::PointViz& viz,
                          const viz::WindowCtx& ctx, viz::MouseButton button,
                          viz::MouseButtonEvent event, viz::EventModifierKeys /*mods*/) {
    if (event == viz::MouseButtonEvent::MOUSE_BUTTON_PRESSED &&
        button == viz::MouseButton::MOUSE_BUTTON_LEFT) {
        return set_pixel_from_viewport_coordinates(img, viz, ctx, ctx.mouse_x, ctx.mouse_y);
    }
    return true;
}

bool mouse_pos_handler(const std::shared_ptr<viz::Image>& img, viz::PointViz& viz,
                       const viz::WindowCtx& ctx, double x, double y) {
    if (ctx.lbutton_down) {
        return set_pixel_from_viewport_coordinates(img, viz, ctx, x, y);
    }
    return true;
}
...
int main(int argc, char* /*unused*/[]) {
    ...
    for (auto& pixel : img_data) {
        pixel = 0.3f * dis3(random_engine);
    }
    img->set_image(IMAGE_WIDTH, IMAGE_HEIGHT, img_data);
    ...
    auto mouse_pos_handler_fn =
        std::bind(mouse_pos_handler, std::ref(img), std::ref(viz), _1, _2, _3);
    viz.push_mouse_pos_handler(mouse_pos_handler_fn);
    auto mouse_button_handler_fn =
        std::bind(mouse_button_handler, std::ref(img), std::ref(viz), _1, _2, _3, _4);
