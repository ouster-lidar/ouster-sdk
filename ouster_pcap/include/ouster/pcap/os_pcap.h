...
struct OUSTER_API_CLASS std::hash<ouster::sdk::pcap::StreamKey> {
    OUSTER_API_FUNCTION
    std::size_t operator()(const ouster::sdk::pcap::StreamKey& key) const noexcept {
        return std::hash<std::string>{}(key.src_ip) ^ (std::hash<std::string>{}(key.src_ip) << 1) ^
               (std::hash<int>{}(key.src_port << 2)) ^ (std::hash<int>{}(key.dst_port << 3));
    }
};
...
struct OUSTER_API_CLASS StreamInfo {
    ...
    std::unordered_map<StreamKey, StreamData> udp_streams;  ///< Datastructure containing info on
...
struct OUSTER_DEPRECATED_MSG("PcapPacketSource",
                             OUSTER_DEPRECATED_LAST_SUPPORTED_1_0) OUSTER_API_CLASS PlaybackHandle;
 ...
 */
OUSTER_DEPRECATED_MSG("PcapPacketSource", OUSTER_DEPRECATED_LAST_SUPPORTED_1_0)
OUSTER_API_FUNCTION
 ...
 */
OUSTER_API_FUNCTION
std::shared_ptr<RecordHandle> record_initialize(const std::string& file, int frag_size,
 ...
 */
OUSTER_API_FUNCTION
std::shared_ptr<StreamInfo> get_stream_info(const std::string& file, int packets_to_process = -1);
...
std::shared_ptr<StreamInfo> get_stream_info(
    ...
    const std::function<void(uint64_t current, uint64_t delta, uint64_t total)>& progress_callback,
 ...
 * @return A pointer to the resulting stream_info
 */
OUSTER_DEPRECATED_MSG("get_stream_info(file, ...)", OUSTER_DEPRECATED_LAST_SUPPORTED_1_0)
OUSTER_API_FUNCTION
std::shared_ptr<StreamInfo> get_stream_info(
