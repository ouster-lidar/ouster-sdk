 ...
 */
struct OUSTER_DEPRECATED_MSG("PcapPacketSource",
                             OUSTER_DEPRECATED_LAST_SUPPORTED_1_0) OUSTER_API_CLASS GlobalIndex {
    uint64_t file_offset;   ///< Byte offset in the PCAP file where the frame starts.
 ...
 */
class OUSTER_DEPRECATED_MSG("PcapFrameSetSource::full_index()",
                            OUSTER_DEPRECATED_LAST_SUPPORTED_1_0) OUSTER_API_CLASS PcapIndex {
   public:
    ...
    std::vector<GlobalIndex> global_frame_indices;  ///< frame index for all sensors, contains the
 ...
 */
enum class OUSTER_DEPRECATED_MSG("PcapPacketSource",
                                 OUSTER_DEPRECATED_LAST_SUPPORTED_1_0) IdxErrorType {
    NONE,
    SIZE,
    ID,
 ...
 */
class OUSTER_DEPRECATED_MSG("PcapPacketSource",
                            OUSTER_DEPRECATED_LAST_SUPPORTED_1_0) OUSTER_API_CLASS IndexedPcapReader
    : public PcapReader {
   public:
    /**
    ...
    OUSTER_API_FUNCTION
    IndexedPcapReader(
        const std::string& pcap_filename,  ///<  [in] - A file path of the pcap to read
        const std::vector<std::string>& metadata_filenames  ///< [in] - A vector of sensor metadata
                                                            ///< file paths
    );
     ...
     */
    OUSTER_API_FUNCTION
    std::pair<IdxErrorType, nonstd::optional<size_t>> check_sensor_idx_for_current_packet(
     ...
     */
    OUSTER_API_FUNCTION
    nonstd::optional<uint32_t> current_frame_id() const;
   ...
   protected:
    ...
    std::vector<nonstd::optional<uint32_t>>
