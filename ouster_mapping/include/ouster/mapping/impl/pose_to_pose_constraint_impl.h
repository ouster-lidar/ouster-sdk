...
class PoseToPoseConstraintImpl : public ConstraintImpl {
   public:
    PoseToPoseConstraintImpl(const std::shared_ptr<Node>& node1, const std::shared_ptr<Node>& node2,
                             Eigen::Quaterniond diff_r, Eigen::Vector3d diff_t,
    ...
    ceres::ResidualBlockId add_to_problem(ceres::Problem& problem,
                                          ceres::LossFunction* loss_func) const override;

    template <typename T>
    bool operator()(const T* const c_i_rotation, const T* const c_i_translation,
                    const T* const c_j_rotation, const T* const c_j_translation, T* residual) const;
