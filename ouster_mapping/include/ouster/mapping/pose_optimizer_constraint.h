 ...
 */
enum class LossFunction { HUBER_LOSS, CAUCHY_LOSS, SOFT_L_ONE_LOSS, ARCTAN_LOSS, TRIVIAL_LOSS };
...
class OUSTER_API_CLASS Constraint {
   public:
     ...
     */
    OUSTER_API_FUNCTION
    Constraint() : constraint_id(next_constraint_id_.fetch_add(1, std::memory_order_relaxed)) {}
    ...
    Constraint(const Eigen::Array3d& translation_weights)
        : translation_weights(translation_weights),
          constraint_id(next_constraint_id_.fetch_add(1, std::memory_order_relaxed)) {}
    ...
    Constraint(const Constraint& other)
        : translation_weights(other.translation_weights), constraint_id(other.constraint_id) {}
     ...
     */
    OUSTER_API_FUNCTION
    uint32_t get_constraint_id() const {
   ...
   private:
    ...
    OUSTER_API_VAR static std::atomic<uint32_t> next_constraint_id_;
...
struct OUSTER_API_CLASS AbsolutePoseConstraint : public Constraint {
     ...
     */
    OUSTER_API_FUNCTION
    AbsolutePoseConstraint(uint64_t timestamp, const core::Matrix4dR& pose,
                           double rotation_weight = 1.0,
...
struct OUSTER_API_CLASS PoseToPoseConstraint : public Constraint {
    ...
    core::Matrix4dR relative_pose = core::Matrix4dR::Identity();
     ...
     */
    OUSTER_API_FUNCTION
    PoseToPoseConstraint(uint64_t timestamp1, uint64_t timestamp2,
                         const core::Matrix4dR& relative_pose = core::Matrix4dR::Identity(),
                         double rotation_weight = 1.0,
                         const Eigen::Array3d& translation_weights = Eigen::Array3d::Ones())
        : Constraint(translation_weights),
...
struct OUSTER_API_CLASS PointToPointConstraint : public Constraint {
    ...
    /** Row index of the first point (staggered image). */
    uint32_t row1;

    /** Column index of the first point (staggered image). */
    uint32_t col1;

    /** Return index of the first point (1 or 2). */
    uint32_t return_idx1;

    /** Row index of the second point (staggered image). */
    uint32_t row2;

    /** Column index of the second point (staggered image). */
    uint32_t col2;
     ...
     * @param[in] row1 Row index of the first point (staggered image).
     * @param[in] col1 Column index of the first point (staggered image).
     * @param[in] return_idx1 Return index of the first point.
     * @param[in] timestamp2 Timestamp of the second point's pose (nanoseconds).
     * @param[in] row2 Row index of the second point (staggered image).
     * @param[in] col2 Column index of the second point (staggered image).
     * @param[in] return_idx2 Return index of the second point.
     * @param[in] translation_weights Optional translation weights.
     */
    OUSTER_API_FUNCTION
    PointToPointConstraint(uint64_t timestamp1, uint32_t row1, uint32_t col1, uint32_t return_idx1,
 ...
 *
 * Constrains a single 3D point from a lidar frame, identified by its 2D image
...
struct OUSTER_API_CLASS AbsolutePointConstraint : public Constraint {
    ...
    /** Row index of the point in the 2D image (staggered). */
    uint32_t row;

    /** Column index of the point in the 2D image (staggered). */
    uint32_t col;
     ...
     * @param[in] row Row index of the 2D image point (staggered).
     * @param[in] col Column index of the 2D image point (staggered).
     * @param[in] return_idx Return index of the point (1 or 2).
     * @param[in] absolute_position Target global 3D position (x, y, z).
     * @param[in] translation_weights Optional translation weights.
     */
    OUSTER_API_FUNCTION
    AbsolutePointConstraint(uint64_t timestamp, uint32_t row, uint32_t col, uint32_t return_idx,
