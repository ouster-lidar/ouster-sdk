...
class OUSTER_API_CLASS PoseOptimizer {
   public:
    ...
    /**
     * @brief Add absolute GPS constraints from POSITION_LAT_LONG and
     * POSITION_TIMESTAMP fields in the source OSF.
     *
     * Generates AbsolutePoseConstraint instances using GPS latitude/longitude
     * samples. Constraints are added at most once every min_space_m meters of
     * travel, estimated from frame poses. If a frame pose is identity, the Z
     * translation is not constrained. Z weight is set to 0 when frame poses are
     * identity. If the source does not contain GPS fields, this call skips
     * constraint generation and returns 0.
     *
     * @param[in] min_space_m Minimum spacing between GPS constraints (meters).
     * Must be > 0.
     * @param[in] translation_weights Translation weights (WX, WY, WZ) for the
     * GPS constraints.
     * @return Number of constraints added.
     * @throws std::invalid_argument if min_space_m <= 0.
     * @throws std::runtime_error if no frames or insufficient data are found.
     */
    OUSTER_API_FUNCTION
    size_t add_absolute_gps_constraints(
        double min_space_m,
        const Eigen::Array3d& translation_weights = Eigen::Array3d(0.01, 0.01, 0.001));
     ...
     *
     * @return The applied 4x4 alignment transform. Returns identity if
     * alignment is skipped (e.g. not enough data, negligible transform).
     */
    OUSTER_API_FUNCTION
     ...
     * frame, useful for fine-grained temporal alignment.
     *
     ...
     *
     * @param[in] func A general callable. Capture what you need.
     */
    OUSTER_API_FUNCTION
    void set_solver_step_callback(std::function<void()> func);
    ...
    /**
     * @brief Retrieve evenly sampled frame nodes with point clouds.
     *
     * @param[in] count Maximum number of samples to retrieve.
     * @return A vector of nodes, each guaranteed to have a downsampled cloud.
     */
    OUSTER_API_FUNCTION
    std::vector<std::shared_ptr<class Node>> get_sampled_nodes(size_t count = 100) const;

    /**
     * @brief Automatically add relative loop-closure constraints using a
     * spatial hash detector.
     *
     * This iterates key-frame poses in order, tracks traveled distance, and
     * adds pose-to-pose constraints between candidates that are spatially
     * close. At most one constraint (closest match) is added per pose. The
     * spatial hash grid uses the provided cell size.
     *
     * @param[in] min_distance_m Minimum traveled distance required between
     * successive loop-closure additions (meters).
     * @param[in] cell_size_m Spatial hash grid cell size (meters).
     * @param[in] icp_score_threshold Minimum auto-ICP confidence score in
     * [0, 1] required to keep an auto-loop pair. Use 0 to disable filtering.
     * @return Number of loop-closure constraints added.
     */
    OUSTER_API_FUNCTION
    size_t add_relative_loop_constraints(double min_distance_m, double cell_size_m,
                                         double icp_score_threshold = 0.0);
...
void save_trajectory(const std::string& filename, const std::vector<uint64_t>& timestamps,
                     const std::vector<core::Matrix4dR>& poses,
