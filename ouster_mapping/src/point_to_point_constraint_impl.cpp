...
namespace mapping {
namespace impl {
PointToPointConstraintImpl::PointToPointConstraintImpl(const std::shared_ptr<Node>& node1,
                                                       const std::shared_ptr<Node>& node2,
                                                       Eigen::Array3d translation_weights)
    : node1_(node1), node2_(node2), translation_weights_(std::move(translation_weights)) {}
...
bool PointToPointConstraintImpl::operator()(const T* const c_i_rotation,
    ...
    // global coordinates).
    Eigen::Vector3d pt_i = node1_->ptp_constraint_pt.row(0);  // Selected point of node1
    Eigen::Vector3d pt_j = node2_->ptp_constraint_pt.row(0);  // Selected point of node2

    // Convert the 3D points to T type for compatibility with Ceres
    const Eigen::Vector3<T> pt_i_t = pt_i.template cast<T>();
    const Eigen::Vector3<T> pt_j_t = pt_j.template cast<T>();

    // Apply the rotation and translation to the points of node1 and node2 to
    // transform them
    Eigen::Quaternion<T> rotation_i(c_i_rotation[3], c_i_rotation[0], c_i_rotation[1],
                                    c_i_rotation[2]);
    Eigen::Quaternion<T> rotation_j(c_j_rotation[3], c_j_rotation[0], c_j_rotation[1],
                                    c_j_rotation[2]);

    // Transform the points by applying the rotations and translations
    Eigen::Vector3<T> transformed_pt_i =
        rotation_i * pt_i_t +
        Eigen::Vector3<T>(c_i_translation[0], c_i_translation[1], c_i_translation[2]);
    Eigen::Vector3<T> transformed_pt_j =
        rotation_j * pt_j_t +
        Eigen::Vector3<T>(c_j_translation[0], c_j_translation[1], c_j_translation[2]);
...
ceres::CostFunction* PointToPointConstraintImpl::create_cost_function() const {
    return new ceres::AutoDiffCostFunction<PointToPointConstraintImpl, 3, 4, 3, 4, 3>(
