...
using ouster::sdk::core::impl::jsonpath_query;
...
class ConstraintParserImpl : public core::impl::JsonTools {
   public:
    ConstraintParserImpl(const jsoncons::json& root, ouster::sdk::core::ValidatorIssues& issues,
                         mapping::SolverConfig& solver_config)
        : JsonTools(root, issues) {
        parse_and_validate_solver_config(solver_config);
    }

    void parse_and_validate_solver_config(mapping::SolverConfig& solver_config) {
        // Parse solver configuration parameters - make them optional with
        // defaults

        if (!parse_and_validate_item(
                issues_.information, "$.key_frame_distance", solver_config.key_frame_distance,
                make_verify_in_bounds<double>(0.0, std::numeric_limits<double>::max()), true)) {
            solver_config.key_frame_distance = 1.0;
            default_message("$.key_frame_distance");
        }

        if (!parse_and_validate_item(
                issues_.information, "$.traj_rotation_weight", solver_config.traj_rotation_weight,
                make_verify_in_bounds<double>(std::numeric_limits<double>::epsilon(),
        ...
        if (!parse_and_validate_item(
                issues_.information, "$.traj_translation_weight",
        ...
        if (!parse_and_validate_item(issues_.information, "$.max_num_iterations",
        ...
        if (!parse_and_validate_item(
                issues_.information, "$.function_tolerance", solver_config.function_tolerance,
                make_verify_in_bounds<double>(0.0, std::numeric_limits<double>::max()))) {
            solver_config.function_tolerance = 1e-6;
            default_message("$.function_tolerance");
        }

        if (!parse_and_validate_item(
                issues_.information, "$.gradient_tolerance", solver_config.gradient_tolerance,
                make_verify_in_bounds<double>(0.0, std::numeric_limits<double>::max()))) {
        ...
        if (!parse_and_validate_item(
                issues_.information, "$.parameter_tolerance", solver_config.parameter_tolerance,
                make_verify_in_bounds<double>(0.0, std::numeric_limits<double>::max()))) {
        ...
        if (!parse_and_validate_enum<std::string>(issues_.information, "$.loss_function",
        ...
        auto parse_constraint_callback = [&](const std::string& constraint_path,
                                             const jsoncons::json& constraint_json) {
            return parse_single_constraint(constraint_path, constraint_json, solver_config);
        };

        // Use vectorized json_query to iterate through constraints array
        jsonpath_query(root_, "$.constraints[*]", parse_constraint_callback);
    }
...
bool ConstraintParserImpl::parse_single_constraint(const std::string& constraint_path,
                                                   const jsoncons::json& constraint_json,
                                                   mapping::SolverConfig& solver_config) {
    if (!constraint_json.is_object()) {
        issues_.critical.emplace_back(constraint_path, "Constraint must be an object");
        return false;
    }

    std::string type_str;
    if (!parse_and_validate_item(issues_.critical, constraint_path + ".type", type_str)) {
        return false;
    }

    try {
        auto parse_weight_vector = [&](const std::string& key, std::vector<double>& output) {
            const std::string path = constraint_path + "." + key;
            if (!path_exists(path)) {
                return false;
            }
            return parse_and_validate_item(issues_.information, path + "[*]", output,
        ...
        auto parse_rotation_weight = [&](double& output) {
            ...
            jsoncons::json value_array = jsonpath_query(root_, path);
            if (value_array.empty()) {
                issues_.critical.emplace_back(path, "rotation_weight is present but empty");
                return false;
            }
            ...
            issues_.critical.emplace_back(path, "rotation_weight must be a single numeric value");
            return false;
        };
        ...
        switch (constraint_type) {
            case mapping::ConstraintType::ABSOLUTE_POSE: {
                auto abs_constraint = std::make_unique<mapping::AbsolutePoseConstraint>();

                // Hard requirements: only timestamp and pose
                if (!parse_and_validate_item(issues_.critical, constraint_path + ".timestamp",
                                             abs_constraint->timestamp)) {
                    return false;
                }

                // Parse pose - handle different formats
                jsoncons::json pose_json = jsonpath_query(root_, constraint_path + ".pose");
                if (pose_json.empty()) {
                    issues_.critical.emplace_back(constraint_path + ".pose",
                ...
                if (pose_value.is_array() && pose_value.size() == 16) {
                    ...
                    std::vector<double> pose_data = pose_value.as<std::vector<double>>();
                ...
                if (parse_weight_vector("translation_weight", trans_weights)) {
                    abs_constraint->translation_weights =
                        Eigen::Array3d(trans_weights[0], trans_weights[1], trans_weights[2]);
                }
            ...
            case mapping::ConstraintType::POSE_TO_POSE: {
                ...
                auto pose_constraint = std::make_unique<mapping::PoseToPoseConstraint>();

                // Parse timestamps
                if (!parse_and_validate_item(issues_.critical, constraint_path + ".timestamp1",
                                             pose_constraint->timestamp1) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".timestamp2",
                ...
                mat4d temp_pose = mat4d::Identity();
                jsoncons::json trans_json =
                    jsonpath_query(root_, constraint_path + ".transformation");
                if (!trans_json.empty()) {
                    auto trans_value = trans_json[0];
                    if (trans_value.is_array() && trans_value.size() == 16) {
                        std::vector<double> trans_data = trans_value.as<std::vector<double>>();
                        JsonTools::decode_transform_array(temp_pose, trans_data);
                        ...
                        issues_.warning.emplace_back(constraint_path + ".transformation",
                ...
                if (parse_weight_vector("translation_weight", trans_weights)) {
                    pose_constraint->translation_weights =
                        Eigen::Array3d(trans_weights[0], trans_weights[1], trans_weights[2]);
                }
            ...
            case mapping::ConstraintType::POINT_TO_POINT: {
                auto pt_constraint = std::make_unique<mapping::PointToPointConstraint>();
                ...
                if (!parse_and_validate_item(issues_.critical, constraint_path + ".timestamp1",
                    ...
                    !parse_and_validate_item(issues_.critical, constraint_path + ".row1",
                                             pt_constraint->row1) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".col1",
                                             pt_constraint->col1) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".return_idx1",
                                             pt_constraint->return_idx1) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".row2",
                                             pt_constraint->row2) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".col2",
                                             pt_constraint->col2) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".return_idx2",
            ...
            case mapping::ConstraintType::ABSOLUTE_POINT: {
                auto abs_pt = std::make_unique<mapping::AbsolutePointConstraint>();

                // Required fields
                if (!parse_and_validate_item(issues_.critical, constraint_path + ".timestamp",
                                             abs_pt->timestamp) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".row",
                                             abs_pt->row) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".col",
                                             abs_pt->col) ||
                    !parse_and_validate_item(issues_.critical, constraint_path + ".return_idx",
                                             abs_pt->return_idx)) {
                    return false;
                }

                jsoncons::json pos_json =
                    jsonpath_query(root_, constraint_path + ".absolute_position");
                if (pos_json.empty()) {
                    issues_.critical.emplace_back(constraint_path + ".absolute_position",
                ...
                if (pos_val.is_object()) {
                    ...
                    abs_pt->absolute_position = Eigen::Vector3d(arr[0], arr[1], arr[2]);
                } else {
            ...
            default:
                issues_.critical.emplace_back(constraint_path + ".type",
...
bool parse_and_validate_constraints(const std::string& json_data, SolverConfig& solver_config,
    ...
    try {
        ...
        issues.critical.emplace_back("$", std::string("Failed to parse JSON: ") + ex.what());
        return false;
    }
}

bool parse_and_validate_constraints(const std::string& json_data, ValidatorIssues& issues) {
    ...
    for (const auto& constraint : solver_config.constraints) {
        ...
        switch (constraint->get_type()) {
            case ConstraintType::ABSOLUTE_POSE: {
                auto abs_constraint = dynamic_cast<const AbsolutePoseConstraint*>(constraint.get());
                if (abs_constraint != nullptr) {
                    ...
                    if (std::abs(abs_constraint->rotation_weight - 1.0) >
                        ...
                        constraint_json["rotation_weight"] = abs_constraint->rotation_weight;
                    }

                    if ((abs_constraint->translation_weights != Eigen::Array3d::Ones()).any()) {
                        jsoncons::json trans_weights = jsoncons::json::array();
                        for (int i = 0; i < 3; ++i) {
                            trans_weights.push_back(abs_constraint->translation_weights(i));
                        }
            ...
            case ConstraintType::POSE_TO_POSE: {
                auto pose_constraint = dynamic_cast<const PoseToPoseConstraint*>(constraint.get());
                if (pose_constraint != nullptr) {
                    ...
                    for (int i = 0; i < 4; ++i) {
                        for (int j = 0; j < 4; ++j) {
                            rel_pose_array.push_back(pose_constraint->relative_pose(i, j));
                        }
                    }
                    ...
                    if (std::abs(pose_constraint->rotation_weight - 1.0) >
                        std::numeric_limits<double>::epsilon()) {
                        constraint_json["rotation_weight"] = pose_constraint->rotation_weight;
                    }

                    if ((pose_constraint->translation_weights != Eigen::Array3d::Ones()).any()) {
                        jsoncons::json trans_weights = jsoncons::json::array();
                        for (int i = 0; i < 3; ++i) {
                            trans_weights.push_back(pose_constraint->translation_weights(i));
                        }
                        constraint_json["translation_weight"] = trans_weights;
                    }
                }
            ...
            case ConstraintType::POINT_TO_POINT: {
                auto pt_constraint = dynamic_cast<const PointToPointConstraint*>(constraint.get());
                if (pt_constraint != nullptr) {
                    ...
                    if ((pt_constraint->translation_weights != Eigen::Array3d::Ones()).any()) {
                        jsoncons::json trans_weights = jsoncons::json::array();
                        for (int i = 0; i < 3; ++i) {
                            trans_weights.push_back(pt_constraint->translation_weights(i));
                        }
                        constraint_json["translation_weight"] = trans_weights;
                    }
                }
            ...
            case ConstraintType::ABSOLUTE_POINT: {
                auto abs_pt = dynamic_cast<const AbsolutePointConstraint*>(constraint.get());
                if (abs_pt != nullptr) {
                    ...
                    if ((abs_pt->translation_weights != Eigen::Array3d::Ones()).any()) {
                        jsoncons::json trans_weights = jsoncons::json::array();
                        for (int i = 0; i < 3; ++i) {
                            trans_weights.push_back(abs_pt->translation_weights(i));
                        }
                        constraint_json["translation_weight"] = trans_weights;
                    }
                }
