...
std::string SensorHttp::get_firmware_version_string(const std::string& hostname, int timeout_sec) {
...
Version SensorHttp::get_firmware_version(const std::string& hostname, int timeout_sec) {
    auto result = get_firmware_version_string(hostname, timeout_sec);
...
std::unique_ptr<SensorHttp> SensorHttp::create(const std::string& hostname, int timeout_sec) {
    auto http_client = std::make_unique<CurlClient>(hostname);
    auto fwjson = http_client->get("api/v1/sensor/metadata/sensor_info", timeout_sec);
    auto parsedjson = jsoncons::json::parse(fwjson);
    // This will exception out on bad parse
    bool is_vlp = parsedjson["prod_line"].as<std::string>().find("VLP") != std::string::npos;
    auto fw = ouster::sdk::core::version_from_string(parsedjson["image_rev"].as<std::string>());

    if (fw == ouster::sdk::core::INVALID_VERSION || fw.major < 2 ||
    ...
    if ((fw.major == 2 && fw.minor == 4) || (fw.major == 3 && fw.minor == 0)) {
        auto instance = std::make_unique<impl::SensorHttpImp_2_4_or_3>(hostname);
