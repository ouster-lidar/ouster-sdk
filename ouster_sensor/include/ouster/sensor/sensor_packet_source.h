...
class OUSTER_API_CLASS Sensor {
   public:
    ...
    OUSTER_API_FUNCTION
    Sensor(std::string hostname,                   ///< [in] sensor hostname
           ouster::sdk::core::SensorConfig config  ///< [in] desired sensor configuration
    );
...
/// SensorFrameSetSource specific configuration options
struct OUSTER_API_CLASS SensorPacketSourceOptions : private PacketSourceOptions {
    ...
    using PacketSourceOptions::rcvbuf_size;
     ...
     */
    OUSTER_API_FUNCTION
    SensorPacketSourceOptions(const PacketSourceOptions& o) : PacketSourceOptions(o) {}
...
class OUSTER_API_CLASS SensorPacketSource
    : public ouster::sdk::core::PacketSource,
      ouster::sdk::impl::PacketSourceBuilderMulti<ouster::sdk::core::IoType::SENSOR,
                                                  SensorPacketSource> {
   public:
    friend class SensorPacketIteratorImpl;
    friend class SensorFrameSetSource;

    /// Construct a sensor packet source for a single sensor
    OUSTER_API_FUNCTION
    SensorPacketSource(const std::string& sensor,         ///< [in] sensor hostname
                       SensorPacketSourceOptions options  ///< [in] source options
    );

    /// Construct a sensor packet source for a multiple sensors
    OUSTER_API_FUNCTION
    SensorPacketSource(const std::vector<std::string>& sources,  ///< [in] sensor hostnames
                       SensorPacketSourceOptions options         ///< [in] source options
    );

    /// Construct a sensor packet source for a single sensor
    OUSTER_API_FUNCTION
    SensorPacketSource(const std::string& source,  ///< [in] sensor hostname
    ...
    OUSTER_API_FUNCTION
    SensorPacketSource(const std::vector<std::string>& source,  ///< [in] sensor hostnames
                       const std::function<void(SensorPacketSourceOptions&)>& options = {}
                       ///< [in] source options
    );
    ...
    SensorPacketSource(
        ...
        double buffer_time_sec = 0  ///< [in] time in seconds to buffer packets for. If zero no
    ...
    OUSTER_API_FUNCTION
    SensorPacketSource(const std::vector<Sensor>& sensors,  ///< [in] sensors to connect to
                       ...
                       bool reuse_ports = false,        ///< [in] If true bind ports with port reuse
                                                        ///< enabled
                       uint32_t rcvbuf_size = 1024 * 1024  ///< [in] Receive buffer size in bytes
    );
    ...
    OUSTER_API_FUNCTION
    ClientEvent get_packet(double timeout_sec  ///< [in] timeout in seconds to wait for a packet
    );
   ...
   private:
    ...
    std::vector<ouster::sdk::core::ImuPacket> imu_packets_;
    std::vector<ouster::sdk::core::LidarPacket> lidar_packets_;
    std::vector<ouster::sdk::core::ZonePacket> zone_packets_;
    ...
    InternalEvent get_packet_internal(std::vector<uint8_t>& data, uint64_t& timestamp,
                                      double timeout_sec);
