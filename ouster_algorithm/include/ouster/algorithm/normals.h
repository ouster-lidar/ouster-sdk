 ...
 *
 * @file
 * @brief Normal values computation for LidarFrame
 */
 ...
 *              ``for (int c = 0; c < frame.w; ++c)`` set
 *              ``sensor_origins.row(c) = (frame.get_column_pose(c) *
 *              frame.sensor_info->sensor_to_body).block<3, 1>(0,
 * 3).transpose();``.
 ...
 */
OUSTER_API_FUNCTION
core::MatrixX3dR normals(const Eigen::Ref<const core::PointCloudXYZd> xyz,
                         const Eigen::Ref<const core::img_t<uint32_t>> range,
                         const Eigen::Ref<const core::MatrixX3dR> sensor_origins_xyz,
 ...
 */
OUSTER_API_FUNCTION
std::pair<core::MatrixX3dR, core::MatrixX3dR> normals(
    const Eigen::Ref<const core::PointCloudXYZd> xyz,
    const Eigen::Ref<const core::img_t<uint32_t>> range,
    const Eigen::Ref<const core::PointCloudXYZd> xyz2,
    const Eigen::Ref<const core::img_t<uint32_t>> range2,
    const Eigen::Ref<const core::MatrixX3dR> sensor_origins_xyz, size_t pixel_search_range = 1,
    double min_angle_of_incidence_rad = DEFAULT_MIN_ANGLE_INCIDENCE_RAD,
    double target_distance_m = DEFAULT_TARGET_DISTANCE_METER);
