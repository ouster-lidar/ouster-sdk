 ...
 * but it is not.
 */
struct OUSTER_API_CLASS PointVizNotRunningError : public std::runtime_error {
    /// Constructor
    OUSTER_API_FUNCTION
    PointVizNotRunningError() : std::runtime_error("PointViz is not running.") {}
};
 ...
 * 4x4 identity matrix
 */
const mat4d IDENTITY4D = mat4d::Identity();
...
// TODO: messes up lidar_frame_viz
namespace impl {
struct CameraData;
class Framebuffer;
class GLCloud;
...
/**
 * @brief Bitmask of types of mouse events to listen to.
 */
enum MouseEventType : int {
    /** Listen to events relevant to left mouse button */
    LEFT = 1,
    /** Listen to events relevant to right mouse button */
    RIGHT = 2,
    /** Listen to events relevant to middle mouse button */
    MIDDLE = 4,
...
class OUSTER_API_CLASS PointViz {
   ...
   public:
    ...
    explicit PointViz(const std::string& name, bool fix_aspect = false,
                      ...
                      int window_height = DEFAULT_WINDOW_HEIGHT, bool maximized = false,
     ...
     * @param[in] mode bitmask of types of events to listen to
     */
    OUSTER_API_FUNCTION
    void push_mouse_button_handler(
        std::function<bool(const WindowCtx& ctx, MouseButton button, MouseButtonEvent event,
                           EventModifierKeys mods)>&& callback,
        MouseEventType mode = MouseEventType::ANY);
     ...
     * Add an object to the scene
     *
     * @param[in] object_overlay Adds an object overlay to the scene
     */
    OUSTER_API_FUNCTION
    void add(const std::shared_ptr<ObjectOverlay>& object_overlay);
     ...
     * Remove an object from the scene
     *
     * @param[in] object_overlay Remove an object overlay from the scene
     *
     * @return true if successfully removed else false
     */
    OUSTER_API_FUNCTION
    bool remove(const std::shared_ptr<ObjectOverlay>& object_overlay);
     ...
     */
    OUSTER_API_FUNCTION
    std::pair<uint32_t, uint32_t> get_scaled_viewport_size(double scale_factor) const;

    /**
     * Try and select points within the specified 2D region. Results are
     * returned via on select callbacks in each selectable object.
     *
     * @param[in] x_1 x coordinate in pixels of first selection point
     * @param[in] y_1 y coordinate in pixels of first selection point
     * @param[in] x_2 x coordinate in pixels of second selection point
     * @param[in] y_2 y coordinate in pixels of second selection point
     */
    OUSTER_API_FUNCTION
    void pick(uint32_t x_1, uint32_t y_1, uint32_t x_2, uint32_t y_2);
   ...
   private:
    ...
    void do_pick(uint32_t x_1, uint32_t y_1, uint32_t x_2, uint32_t y_2);
     ...
     * be read.
     */
    std::vector<uint8_t> capture_framebuffer_pixels(impl::Framebuffer& framebuffer);
...
struct OUSTER_API_CLASS WindowCtx {
    ...
    bool rbutton_down{false};  ///< True if the right mouse button is held
...
class OUSTER_API_CLASS TargetDisplay {
   ...
   public:
     ...
     *
     * @param[in] ring_size space between rings will be 10^n meters
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @return distance between rings in meters
     */
    OUSTER_API_FUNCTION
 ...
 */
class OUSTER_API_CLASS Selectable {
    friend class PointViz;

   protected:
    std::function<void(const std::vector<uint32_t>&)> on_select_{};
    OUSTER_API_FUNCTION
    virtual ~Selectable() = default;

   public:
    /**
     * Set the selection callback. Argument to the callback is a list of
     * indexes selected.
     *
     * @param[in] callback function to call when points are selected
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @return true if there is a selection callback
     */
    OUSTER_API_FUNCTION
    bool selectable() {
        return static_cast<bool>(on_select_);
    }
};
 ...
 */
class OUSTER_API_CLASS Cloud : public Selectable {
   protected:
    size_t n_{0};
    size_t w_{0};
    mat4d extrinsic_{};
    ...
    std::shared_ptr<impl::BufferReference> range_data_{};
    std::shared_ptr<impl::BufferReference> key_data_{};
    std::shared_ptr<impl::BufferReference> mask_data_{};
    std::shared_ptr<impl::BufferReference> xyz_data_{};
    std::shared_ptr<impl::BufferReference> off_data_{};
    std::shared_ptr<impl::TextureReference> transform_data_{};
    std::shared_ptr<impl::TextureReference> palette_data_{};
    mat4d pose_{};
    float point_size_{2};
    bool mono_{true};
    uint32_t viz_instance_{0};
   ...
   public:
     ...
     * @param[in] extrinsic sensor extrinsic calibration. 4x4 row-major
     ...
     * Call set_range() to update
     *
     * @param[in] sensor_info information of sensor the range data came from
     */
    OUSTER_API_FUNCTION
     ...
     * Call set_range() to update
     *
     * @param[in] xyz_lut XYZLut to transform from range data to 3D
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @param[in] key_rgb pointer to array of at least 3x as many elements as
     * there are points, between 0 and 255
     */
    OUSTER_API_FUNCTION
     ...
     * points, so that the xyz position of the ith point is i, i + 3, i + 6
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @param[in] pose 4x4 row-major homogeneous transformation matrix
     */
    OUSTER_API_FUNCTION
     ...
     * pixel at row u, column v in the staggered lidar frame is transformed
...
class OUSTER_API_CLASS Image {
    ...
    std::shared_ptr<impl::TextureReference> image_data_{};
    size_t mask_width_{0};
    size_t mask_height_{0};
    std::shared_ptr<impl::TextureReference> mask_data_{};
   ...
   public:
     ...
     */
    OUSTER_API_FUNCTION
    std::pair<int, int> viewport_coordinates_to_image_pixel(const WindowCtx& ctx, double x,
                                                            double y) const;
     ...
     */
    OUSTER_API_FUNCTION
    std::pair<double, double> image_pixel_to_viewport_coordinates(const WindowCtx& ctx, int px,
                                                                  int py) const;
...
/**
 * @brief Manages the state of an object overlay.
 */
class OUSTER_API_CLASS ObjectOverlay {
    bool position_changed_{false};
    bool objects_changed_{false};
    bool sensor_info_changed_{false};
    bool view_matrix_changed_{false};

    vec4f position_{};
    float hshift_{0};
    mat4d view_matrix_{IDENTITY4D};
   ...
   public:
     ...
     *
     * @param[in] cuboids vector of Cuboid objects to display
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @param[in] mat 4x4 row-major matrix
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @param[in] x_min min x
     * @param[in] x_max max x
     * @param[in] y_min min y
     * @param[in] y_max max y
     */
    OUSTER_API_FUNCTION
    void set_position(float x_min, float x_max, float y_min, float y_max);
...
class OUSTER_API_CLASS Mesh {
    ...
    mat4d transform_{};
    std::shared_ptr<std::vector<Vertex3f>> vertices_;
    std::shared_ptr<std::vector<unsigned int>> face_indices_;
    std::shared_ptr<std::vector<unsigned int>> edge_indices_;
   ...
   public:
     ...
     */
    OUSTER_API_FUNCTION
    Mesh(const std::vector<Vertex3f>& vertices, const std::vector<unsigned int>& face_indices,
 ...
 */
class OUSTER_API_CLASS Cuboid : public Selectable {
    bool transform_changed_{false};
    bool rgba_changed_{false};
   ...
   public:
     ...
     *
     * @param[in] object the ouster::sdk::core::Object to construct from
     * @param[in] rgba color to use for the cuboid, in RGBA format
     * @return a new Cuboid representing the object
     */
    OUSTER_API_FUNCTION
     ...
     *
     * @return 4x4 matrix representing the cuboid transform
     */
    OUSTER_API_FUNCTION
    const mat4d& get_transform() const {
        return transform_;
    }  // TODO de-inline
     ...
     *
     * @return rgba color of the cuboid
     */
    OUSTER_API_FUNCTION
    const vec4f& get_rgba() const {
        return rgba_;
    }  // TODO de-inline
...
class OUSTER_API_CLASS Lines {
    ...
    mat4d transform_{};
    vec4f rgba_{};
    std::shared_ptr<std::vector<float>> point_data_;
...
/**
 * Distinct color palette number of colors.
 */
extern OUSTER_API_VAR const size_t DISTINCT_N;

/**
 * Distinct color palette, RGB values per element.
 */
extern OUSTER_API_VAR const float DISTINCT_PALETTE[][3];

/**
 * Spezia palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t SPEZIA_N;
/**
 * Spezia palette, RGB values per element.
 */
extern OUSTER_API_VAR const float SPEZIA_PALETTE[][3];

/**
 * Spezia Cal Ref palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t SPEZIA_CAL_REF_N;
/**
 * Spezia Cal Ref palette, RGB values per element.
 */
extern OUSTER_API_VAR const float SPEZIA_CAL_REF_PALETTE[][3];

/**
 * Calibrated reflectifiy palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t CALREF_N;
/**
 * Calibrated reflectifiy, RGB values per element.
 */
extern OUSTER_API_VAR const float CALREF_PALETTE[][3];

/**
 * Greyscale palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t GREY_N;
/**
 * Greyscale palette, RGB values per element.
 */
extern OUSTER_API_VAR const float GREY_PALETTE[][3];

/**
 * Greyscale Cal Ref palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t GREY_CAL_REF_N;
/**
 * Greyscale Cal Ref palette, RGB values per element.
 */
extern OUSTER_API_VAR const float GREY_CAL_REF_PALETTE[][3];

/**
 * Viridis palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t VIRIDIS_N;
/**
 * Viridis palette, RGB values per element.
 */
extern OUSTER_API_VAR const float VIRIDIS_PALETTE[][3];

/**
 * Viridis Cal Ref palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t VIRIDIS_CAL_REF_N;
/**
 * Viridis Cal Ref palette, RGB values per element.
 */
extern OUSTER_API_VAR const float VIRIDIS_CAL_REF_PALETTE[][3];

/**
 * Magma palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t MAGMA_N;
/**
 * Magma palette, RGB values per element.
 */
extern OUSTER_API_VAR const float MAGMA_PALETTE[][3];

/**
 * Magma Cal Ref palette size in number of colors.
 */
extern OUSTER_API_VAR const size_t MAGMA_CAL_REF_N;
/**
 * Magma Cal Ref palette, RGB values per element.
 */
extern OUSTER_API_VAR const float MAGMA_CAL_REF_PALETTE[][3];
