...
#include "ouster/core/lidar_frame.h"  // NOLINT(misc-include-cleaner,unused-includes) needed for destagger
...
// beam angle.
Eigen::Vector3f direction_for_pixel(const SensorInfo& sensor_info, double row, double col) {
    double theta_encoder = 2.0 * M_PI * (1.0 - col / sensor_info.w());
    double phi = M_PI * lerp_vector(sensor_info.beam_altitude_angles, row) / 180.0;
    Eigen::Vector3f direction(static_cast<float>(std::cos(theta_encoder) * std::cos(phi)),
...
Eigen::Vector3f direction_rot(const SensorInfo& sensor_info, double row, double col) {
    auto dir = direction_for_pixel(sensor_info, row, col);
    auto& rot = sensor_info.lidar_to_sensor_transform.topLeftCorner(3, 3).cast<float>();
...
Eigen::Vector3f offset(const SensorInfo& sensor_info, double row, double col) {
    double beam_to_lidar_euclidean_distance_mm = std::sqrt(
        (sensor_info.beam_to_lidar_transform(0, 3) * sensor_info.beam_to_lidar_transform(0, 3)) +
        (sensor_info.beam_to_lidar_transform(2, 3) * sensor_info.beam_to_lidar_transform(2, 3)));

    double theta_encoder = 2.0 * M_PI * (1.0 - col / sensor_info.w());
    Eigen::Vector3f dir = direction_for_pixel(sensor_info, row, col);

    Eigen::Vector3f offset_v(
        static_cast<float>((std::cos(theta_encoder) * sensor_info.beam_to_lidar_transform(0, 3)) -
                           (dir.x() * beam_to_lidar_euclidean_distance_mm)),
        static_cast<float>((std::sin(theta_encoder) * sensor_info.beam_to_lidar_transform(0, 3)) -
    ...
    auto translation = sensor_info.lidar_to_sensor_transform.topRightCorner(3, 1).cast<float>();
...
VertexLookupTable precompute_voxel_vertices(const SensorInfo& metadata) {
    ...
    VertexLookupTable lookup_table(n_rows + 1, std::vector<VoxelVertexData>(n_cols + 1));

    for (int row = 0; row <= n_rows; ++row) {
        for (int col = 0; col <= n_cols; ++col) {
            // Each (row, col) in this grid corresponds to the geometric point
            // (j-0.5, i-0.5)
            lookup_table[row][col] = {direction_rot(metadata, row - 0.5, col - 0.5),
...
voxel_style_mesh_components_from_range_images_with_lut(
    const ouster::sdk::core::img_t<uint32_t>& near_range_image_mm,
    const ouster::sdk::core::img_t<uint32_t>& far_range_image_mm, const SensorInfo& metadata,
    const VertexLookupTable& vertex_lookup, bool add_faces, bool add_edges) {
    auto normalize = [](const Eigen::Vector3f& vec) { return vec / vec.norm(); };
    ...
    vector<map<pair<int, int>, vector<Vertex3f>>> verts = {map<pair<int, int>, vector<Vertex3f>>(),
    ...
    const array<decltype(near_destaggered), 2> destaggered_faces = {near_destaggered,
                                                                    far_destaggered};
        ...
        for (int col = 1; col < n_cols - 1; col++) {
            ...
            for (size_t face_idx = 0; face_idx < 2; ++face_idx) {
                ...
                vector<Vertex3f> new_verts = {
                    Vertex3f(rng * corners[0]->direction + corners[0]->offset, face_normal),
                    Vertex3f(rng * corners[1]->direction + corners[1]->offset, face_normal),
                    Vertex3f(rng * corners[2]->direction + corners[2]->offset, face_normal),
                    Vertex3f(rng * corners[3]->direction + corners[3]->offset, face_normal)};
                ...
                if (add_edges) {
                    edges.insert(edges.end(), {num_vertices + 0, num_vertices + 1, num_vertices + 1,
    ...
    for (size_t face_idx = 0; face_idx < 2; ++face_idx) {
        for (const auto& pixel_and_vertex : verts[face_idx]) {
            ...
            auto& verts_a = pixel_and_vertex.second;
            const int row = pixel.first;
            const int col = pixel.second;

            if (verts[face_idx].count({row, col + 1}) > 0) {  // Right neighbor
                const auto& b = verts[face_idx].at({row, col + 1});
                auto normal = normalize((verts_a[2].position - verts_a[1].position)
                                            .cross(b[0].position - verts_a[1].position));
                vector<Vertex3f> new_verts = {
                    Vertex3f(verts_a[1].position, normal), Vertex3f(b[0].position, normal),
                    Vertex3f(b[3].position, normal), Vertex3f(verts_a[2].position, normal)};
                vertices.insert(vertices.end(), new_verts.begin(), new_verts.end());
                if (add_faces) {
                    faces.insert(faces.end(),
                ...
                if (add_edges) {
                    edges.insert(edges.end(), {num_vertices + 0, num_vertices + 1, num_vertices + 2,
                                               num_vertices + 3});
                }
                num_vertices += static_cast<uint32_t>(new_verts.size());
            }

            if (verts[face_idx].count({row + 1, col}) > 0) {  // Bottom neighbor
                const auto& b = verts[face_idx].at({row + 1, col});
                auto normal = normalize((b[0].position - verts_a[3].position)
                                            .cross(b[1].position - verts_a[3].position));
                vector<Vertex3f> new_verts = {
                    Vertex3f(verts_a[3].position, normal), Vertex3f(verts_a[2].position, normal),
                    Vertex3f(b[0].position, normal), Vertex3f(b[1].position, normal)};
                vertices.insert(vertices.end(), new_verts.begin(), new_verts.end());
                if (add_faces) {
                    faces.insert(faces.end(),
                ...
                if (add_edges) {
                    edges.insert(edges.end(), {num_vertices + 0, num_vertices + 2, num_vertices + 1,
                                               num_vertices + 3});
                }
                num_vertices += static_cast<uint32_t>(new_verts.size());
            }
    ...
    auto add_connecting_wall = [&](const vector<Vertex3f>& new_verts) {
        ...
        if (add_faces) {
            faces.insert(faces.end(), {num_vertices + 0, num_vertices + 1, num_vertices + 2,
                                       num_vertices + 2, num_vertices + 3, num_vertices + 1});
        }
        num_vertices += static_cast<uint32_t>(new_verts.size());
    };
    ...
    for (const auto& pixel_and_vertex : verts[0]) {
        ...
        if (verts[0].count({row - 1, col}) == 0) {  // Top wall
            auto normal = normalize((verts_a[1].position - verts_a[0].position)
                                        .cross(b[1].position - verts_a[0].position));
            add_connecting_wall({Vertex3f(verts_a[0].position, normal),
                                 Vertex3f(verts_a[1].position, normal),
                                 Vertex3f(b[0].position, normal), Vertex3f(b[1].position, normal)});
        }
        if (verts[0].count({row + 1, col}) == 0) {  // Bottom wall
            auto normal = normalize((verts_a[2].position - verts_a[3].position)
                                        .cross(b[2].position - verts_a[3].position));
            add_connecting_wall({Vertex3f(b[2].position, normal), Vertex3f(b[3].position, normal),
                                 Vertex3f(verts_a[2].position, normal),
                                 Vertex3f(verts_a[3].position, normal)});
        }
        if (verts[0].count({row, col - 1}) == 0) {  // Left wall
            auto normal = normalize((verts_a[3].position - verts_a[0].position)
                                        .cross(b[3].position - verts_a[0].position));
            add_connecting_wall({Vertex3f(verts_a[0].position, normal),
                                 Vertex3f(verts_a[3].position, normal),
                                 Vertex3f(b[0].position, normal), Vertex3f(b[3].position, normal)});
        }
        if (verts[0].count({row, col + 1}) == 0) {  // Right wall
            auto normal = normalize((verts_a[2].position - verts_a[1].position)
                                        .cross(b[2].position - verts_a[1].position));
            add_connecting_wall({Vertex3f(verts_a[1].position, normal),
                                 Vertex3f(verts_a[2].position, normal),
                                 Vertex3f(b[1].position, normal), Vertex3f(b[2].position, normal)});
        }
    }
...
ouster::sdk::viz::Mesh voxel_style_mesh_from_zone_image_pair(
    ...
    auto res = voxel_style_mesh_components_from_range_images_with_lut(
        zone_image_pair.near_range_mm, zone_image_pair.far_range_mm, sensor_info, vertex_lookup,
        false, true);
    return ouster::sdk::viz::Mesh(std::move(std::get<0>(res)), std::move(std::get<1>(res)),
