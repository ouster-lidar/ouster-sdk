...
// Reads OpenGL framebuffer state; not logically const despite no member writes.
// NOLINTNEXTLINE(readability-make-member-function-const)
void Framebuffer::read_pixels_into(std::vector<uint8_t>& buffer) const {
    if (!bound_) {
        throw std::runtime_error("Read framebuffer while not bound");
    }
    if (buffer.size() < static_cast<std::size_t>(width_) * static_cast<std::size_t>(height_) * 3) {
        throw std::runtime_error("Buffer not large enough for reading from framebuffer");
    }

    glReadPixels(0, 0, width_, height_, GL_RGB, GL_UNSIGNED_BYTE, buffer.data());
}
