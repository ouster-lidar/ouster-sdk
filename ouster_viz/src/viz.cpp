...
struct VizHandle {
    ...
    LidarScanBuffer lsb;
    const ouster::xyz_lut* xyz_lut;
    std::string prod_line;
    OS1::data_format format;
    std::vector<double> range_markers;
...
void update(viz::VizHandle& vh, std::unique_ptr<LidarScan>& ls) {
    assert(ls->w * ls->h ==
           vh.format.columns_per_frame * vh.format.pixels_per_column);
...
void update_color_key(
    const VisualizerConfig& config, VisualizerState& state,
    const LidarScan::Points& xyz,
...
void update_images(
    ...
    const std::vector<uint32_t>& px_offset, const VisualizerConfig& config,
    VisualizerState& state) {
    ...
    for (int u = 0; u < ls.h; u++) {
        ...
        dst.row(1 * ls.h - u - 1)
            << range.row(u).tail(ofs).cast<double>(),
            range.row(u).head(ls.w - ofs).cast<double>();
        dst.row(2 * ls.h - u - 1)
            << intensity.row(u).tail(ofs).cast<double>(),
            intensity.row(u).head(ls.w - ofs).cast<double>();
        dst.row(3 * ls.h - u - 1)
            << noise.row(u).tail(ofs).cast<double>(),
            noise.row(u).head(ls.w - ofs).cast<double>();
    }
...
std::vector<vtkSmartPointer<vtkProp>> init_grid_actors(
    ...
    labels->SetNumberOfValues(markers.size());
    labels->SetName("labels");

    vtkSmartPointer<vtkIntArray> sizes = vtkSmartPointer<vtkIntArray>::New();
    sizes->SetNumberOfValues(markers.size());
    sizes->SetName("sizes");
    ...
    for (size_t i = 0; i < markers.size(); i++) {
        ...
        polygonSource->SetRadius(markers[i]);
        ...
        points->InsertNextPoint(0.0, markers[i], 0.0);

        std::ostringstream out;
        out.precision(1);
        out << std::fixed << markers[i] << "m";
...
void run_viz(VizHandle& vh) {
    const auto w = vh.format.columns_per_frame;
    const auto h = vh.format.pixels_per_column;

    const LidarScan::index_t n_points = w * h;

    Eigen::Array<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        image_data{3 * h, w};

    LidarScan::Points pc_render{n_points, 3};
    Eigen::Array<double, Eigen::Dynamic, 3, Eigen::RowMajor> pc_render_rowmajor{
        n_points, 3};
    std::vector<double> color_key(n_points, 0.0);

    const auto px_offset = vh.format.pixel_shift_by_row;
    ...
    if (vh.prod_line.find("OS-0-") == 0)
        vfov = 90.0;
    else if (vh.prod_line.find("OS-1-") == 0)
        vfov = 40.0;
    else if (vh.prod_line.find("OS-2-") == 0)
        vfov = 22.5;
    image_change->SetOutputSpacing(vh.format.pixels_per_column / vfov,
                                   vh.format.columns_per_frame / 360.0, 1.0);
    image_change->Update();
    ...
    auto grid_actors = init_grid_actors(vh.range_markers);
 ...
 **/
std::shared_ptr<VizHandle> init_viz(const ouster::xyz_lut* xyz_lut,
                                    const OS1::data_format& format,
                                    const std::string& prod_line,
                                    const std::vector<double>& range_markers) {
    ...
    vh->xyz_lut = xyz_lut;
    vh->exit = false;
    vh->range_markers = range_markers;
    ...
    vh->lsb.back = std::unique_ptr<LidarScan>(
        new LidarScan(format.columns_per_frame, format.pixels_per_column));
    vh->lsb.front = std::unique_ptr<LidarScan>(
        new LidarScan(format.columns_per_frame, format.pixels_per_column));
