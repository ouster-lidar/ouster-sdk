...
GLCloud::GLCloud(const Cloud& cloud) : point_size_{cloud.point_size_} {
    ...
    glGenBuffers(1, &off_buffer_);
    glGenBuffers(1, &range_buffer_);
    glGenBuffers(1, &key_buffer_);
    glGenBuffers(1, &mask_buffer_);
    glGenBuffers(1, &trans_index_buffer_);
    glGenTextures(1, &transform_texture_);
...
GLCloud::~GLCloud() {
    ...
    glDeleteBuffers(1, &off_buffer_);
    glDeleteBuffers(1, &range_buffer_);
    glDeleteBuffers(1, &key_buffer_);
    glDeleteBuffers(1, &mask_buffer_);
    glDeleteBuffers(1, &trans_index_buffer_);
    glDeleteTextures(1, &transform_texture_);
    glDeleteTextures(1, &palette_texture_);
    glDeleteVertexArrays(1, &vao_);
}

/**
 * @brief Makes a key from the pair of (cloud_size, w) for use in maps.
 *
 * @param[in] cloud_size total cloud size
 * @param[in] w columns number
 * @return size_t hash of the pair (cloud_size, w) to use as a map key
 */
static inline size_t ti_key(size_t cloud_size, size_t w) {
...
void GLCloud::initialize_vao() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, mask_buffer_);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, off_buffer_);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, trans_index_buffer_);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, range_buffer_);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, key_buffer_);
 ...
 */
void GLCloud::draw(const WindowCtx& /*unused*/, const CameraData& camera,
                   Cloud& cloud) {
    glBindVertexArray(vao_);
    ...
    if (it == trans_indexes.end()) {
        ...
        for (size_t i = 0; i < cloud.n_; i++) {
            trans_index_buffer_data[i] =
                ((i % cloud.w_) + 0.5) / static_cast<GLfloat>(cloud.w_);
        }
    ...
    if (cloud.pose_changed_) {
        map_pose_ = Eigen::Map<const Eigen::Matrix4d>{cloud.pose_.data()};
        cloud.pose_changed_ = false;
    }
    extrinsic_ = Eigen::Map<const Eigen::Matrix4d>{cloud.extrinsic_.data()}
                     .cast<float>();

    const Eigen::Matrix4f mvp =
        (camera.proj * camera.view * camera.target * map_pose_).cast<float>();
    ...
    if (cloud.palette_changed_) {
        load_texture(cloud.palette_data_->data(),
                     cloud.palette_data_->size() / 3, 1, palette_texture_);
    ...
    if (cloud.transform_changed_) {
        load_texture(cloud.transform_data_->data(), cloud.w_, 4,
                     transform_texture_, GL_RGB32F);
        cloud.transform_changed_ = false;
    }
    glBindTexture(GL_TEXTURE_2D, transform_texture_);
    ...
    glUniform1i(GLCloud::cloud_ids.mono_id, mono_ ? 1 : 0);
