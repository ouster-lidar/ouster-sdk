...
SOCKET mtp_data_socket(int port, const std::string& mtp_group = "", 
    ...
    for (ai = info_start; ai != NULL; ai = ai->ai_next) {
        ...
        if (!impl::socket_valid(sock_fd)) {
            std::cerr << "mtp socket(): " << impl::socket_get_error()
                      << std::endl;
            continue;
        }

        u_int off = 1;
        if (setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, (char*)&off,
                       sizeof(off))) {
            std::cerr << "mtp setsockopt(): " << impl::socket_get_error()
                      << std::endl;
            impl::socket_close(sock_fd);
            return SOCKET_ERROR;
        }
        ...
        if (bind(sock_fd, ai->ai_addr, (socklen_t)ai->ai_addrlen)) {
            impl::socket_close(sock_fd);
            std::cerr << "mtp bind(): " << impl::socket_get_error()
                      << std::endl;
            continue;
        }
    ...
    if (!impl::socket_valid(impl::socket_set_non_blocking(sock_fd))) {
        std::cerr << "mtp fcntl(): " << impl::socket_get_error() << std::endl;
        impl::socket_close(sock_fd);
        return SOCKET_ERROR;
    }
...
std::shared_ptr<client> init_client(const std::string& hostname,
                                    ...
                                    const std::string& mtp_dest_host,
                                    int timeout_sec) {
    auto cli = init_client(hostname, udp_dest_host, mtp_dest_host, lidar_port, imu_port);
