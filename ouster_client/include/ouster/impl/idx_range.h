...
OSDK_FN std::enable_if_t<!is_idx_range_t<U>::V, T> range_replace_dim(
    T dim, U /*unused*/) {
    return dim;
}
...
OSDK_FN void range_args_restride(const IntT (&strides)[N],
                                 IntT (&new_strides)[M], Args... /*unused*/) {
 ...
 */
template <typename IntT, int N, int M, typename... Args>
OSDK_FN void range_args_reshape(const IntT (&shape)[N], IntT (&new_shape)[M],
                                Args... args) {
    int i = 0;
