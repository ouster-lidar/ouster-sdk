   ...
   public:
    FileSha(const std::string& filename)
        : context(EVP_MD_CTX_new()),
          ...
          fsize(ouster::sdk::osf::file_size(filename)),
          digest{0},
        ...
        uint64_t i = ouster::sdk::osf::file_size(filename);
        bool finished = false;
...
TEST_F(OperationsTest, BackupMetadataTest) {
    // It should throw if trying to restore metadata in an invalid OSF
    ...
    // The OSF is invalid after this point, because we wrote an invalid metadata
    // blob to it.
...
TEST_F(OperationsTest, BackupMetadataTest2) {
    // Backing up and restoring the metadata should result in a file with the
    // same hash.
    std::string osf_file_path =
        path_concat(test_data_dir(), "osfs/OS-1-128_v2.3.0_1024x10_lb_n3.osf");
    std::string temp_dir;
    ...
    restore_osf_file_metablob(temp_file, temp_backup);
    StreamOsfFile osf_file{temp_file};
    EXPECT_TRUE(osf_file.good());
    EXPECT_EQ(osf_file.version(), ouster::sdk::core::Version(2, 1, 0));
    auto sha2 = FileSha(temp_file).get_string();
    EXPECT_EQ(sha1, sha2);
}
...
ouster::sdk::core::SensorInfo _gen_new_metadata(int start_number) {
    ouster::sdk::core::SensorInfo new_metadata;
    new_metadata.sn = 123456;
    new_metadata.fw_rev = "sqrt(-1) friends";
    new_metadata.config.lidar_mode = ouster::sdk::core::LidarMode::_512x10;
...
TEST_F(OperationsTest, MetadataRewriteTestMulti) {
    ...
    ouster::sdk::core::SensorInfo new_metadata = _gen_new_metadata(100);
    ouster::sdk::core::SensorInfo new_metadata2 = _gen_new_metadata(200);
