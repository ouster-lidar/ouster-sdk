...
using ouster::sdk::core::impl::kmp_search;
...
TEST(Mesh, load_from_ascii_invalid) {
    std::string data_dir = getenvs("DATA_DIR");
    Mesh mesh;
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_expected_vertex.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_expected_endloop.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_expected_outer_loop.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_expected_endfacet.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_empty.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_expected_solid.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_expected_endsolid.stl"));
    EXPECT_FALSE(mesh.load_from_stl(data_dir + "/ascii_invalid_unexpected_line.stl"));
}
...
TEST(Mesh, solidworks) {
    // it correctly parses an STL file generated by Solidworks, which includes
    // "solid" in the header
    std::string data_dir = getenvs("DATA_DIR");
    Mesh mesh;
    ASSERT_TRUE(mesh.load_from_stl(data_dir + "/solidworks_binary.stl"));
    ASSERT_EQ(mesh.triangles().size(), 12);
}

TEST(Mesh, kmp_search) {
    std::stringstream input_stream("This is a test string for KMP search.");
    auto position = kmp_search(input_stream, "test string");
    EXPECT_EQ(position, 10);  // "test string" starts at index 10
    position = kmp_search(input_stream, "Est");
    EXPECT_EQ(position, 11);  // it's case insensitive
    position = kmp_search(input_stream, "not found");
    EXPECT_EQ(position, std::string::npos);  // "test string" starts at index 10
}

TEST(Mesh, save_stl_binary) {
    std::string data_dir = getenvs("DATA_DIR");
    Mesh mesh;
    ASSERT_TRUE(mesh.load_from_stl(data_dir + "/0.stl"));
    std::stringstream stream;
    mesh.save_stl_binary(stream);
    Mesh loaded_mesh;
    ASSERT_TRUE(loaded_mesh.load_from_stl_stream(stream));
    EXPECT_EQ(mesh, loaded_mesh);
}
