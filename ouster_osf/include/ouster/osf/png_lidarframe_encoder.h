...
/**
 * @brief Encoder for lidar frame fields into PNG format.
 *
 * This class implements the LidarFrameEncoder interface to encode lidar frame
 * fields into PNG format, supporting various bit depths and destaggering
 * options.
 */
class OUSTER_API_CLASS PngLidarFrameEncoder : public ouster::sdk::osf::LidarFrameEncoder {
   public:
    /**
     * @brief Construct a PngLidarFrameEncoder with a specified compression
     * level.
     *
     ...
     */
    OUSTER_API_FUNCTION
    PngLidarFrameEncoder(int compression_amount) : compression_amount_{compression_amount} {}
   ...
   private:
     ...
     */
    template <typename T>
    bool encode_8bit_image(FrameChannelData& res_buf,
     ...
     */
    template <typename T>
    bool encode_16bit_image(FrameChannelData& res_buf,
                            const Eigen::Ref<const ouster::sdk::core::img_t<T>>& img,
                            const std::vector<int>& px_offset) const;

    /**
     * Encode 2D image of a typical lidar frame field channel into a 16 bit,
     * Gray, PNG buffer.
     *
     * @tparam T The type to use for the output array.
     *
     ...
     * Encode 2D image of a typical lidar frame field channel into a 32 bit,
     * RGBA, PNG buffer.
     *
     * @tparam T The type to use for the output array.
     *
     * @param[out] res_buf The output buffer with a single encoded PNG.
     * @param[in] img 2D image or a single LidarFrame field data.
     * @return false (0) if operation is successful, true (1) if error occured
     */
     ...
     */
    template <typename T>
    bool encode_32bit_image(FrameChannelData& res_buf,
                            const Eigen::Ref<const ouster::sdk::core::img_t<T>>& img,
                            const std::vector<int>& px_offset) const;
     ...
     * Encode 2D image of a typical lidar frame field channel into a 24 bit,
     * RGB, PNG buffer.
     *
     * @tparam T The type to use for the output array.
     *
     ...
     */
    template <typename T>
    bool encode_24bit_image(FrameChannelData& res_buf,
                            const Eigen::Ref<const ouster::sdk::core::img_t<T>>& img,
                            const std::vector<int>& px_offset) const;
     ...
     * Encode 2D image of a typical lidar frame field channel into a 64 bit,
     * RGBA, PNG buffer.
     *
     * @tparam T The type to use for the output array.
     *
     ...
     */
    template <typename T>
    bool encode_64bit_image(FrameChannelData& res_buf,
                            const Eigen::Ref<const ouster::sdk::core::img_t<T>>& img,
                            const std::vector<int>& px_offset) const;
...
OUSTER_DEPRECATED_TYPE(PngLidarScanEncoder, PngLidarFrameEncoder,
