 ...
 */
struct VectorReader {
    const EncodedFrameChannelData& buffer;
    uint32_t offset = 0;
    explicit VectorReader(const EncodedFrameChannelData& buf) : buffer(buf) {}
...
void png_osf_error(png_structp png_ptr, png_const_charp msg) {
    ...
    // libpng error callback; exceptions cannot propagate through C callbacks.
    // NOLINTNEXTLINE(cert-err52-cpp)
    longjmp(png_jmpbuf(png_ptr), 1);
};

/** @internal */
inline void print_incompatable_image_size(png_uint_32 actual_width, png_uint_32 actual_height,
                                          png_uint_32 expected_width, png_uint_32 expected_height) {
    logger().error(
 ...
 */
void png_osf_write_data(png_structp png_ptr, png_bytep bytes, png_size_t bytes_len) {
 ...
 * helper)
 */
void png_osf_read_data(png_structp png_ptr, png_bytep bytes, png_size_t bytes_len) {
    VectorReader* vec_read = reinterpret_cast<VectorReader*>(png_get_io_ptr(png_ptr));
...
bool png_osf_write_init(png_structpp png_ptrp, png_infopp png_info_ptrp) {
    *png_ptrp =
        png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr, png_osf_error, png_osf_error);
    if (*png_ptrp == nullptr) {
...
bool png_osf_read_init(png_structpp png_ptrp, png_infopp png_info_ptrp) {
    *png_ptrp =
        png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, png_osf_error, png_osf_error);
    if (*png_ptrp == nullptr) {
        logger().error("ERROR: no png_ptr");
        return true;
    }
...
void png_osf_write_start(png_structp png_ptr, png_infop png_info_ptr, FrameChannelData& res_buf,
    ...
    png_set_IHDR(png_ptr, png_info_ptr, width, height, sample_depth, color_type, PNG_INTERLACE_NONE,
...
template <typename T>
bool decode_24bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf,
...
template bool decode_24bit_image<uint8_t>(Eigen::Ref<img_t<uint8_t>>,
                                          const EncodedFrameChannelData&, const std::vector<int>&);
...
bool decode_24bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf) {
    ...
    if (width != static_cast<png_uint_32>(img.cols()) ||
        ...
        print_incompatable_image_size(width, height, static_cast<png_uint_32>(img.cols()),
    ...
    // 24bit channel data decoding to LidarFrame for key channel_index
    for (size_t row = 0; row < height; row++) {
        for (size_t col = 0; col < width; col++) {
            img(row, col) = static_cast<T>(row_pointers[row][(col * 3) + 0]) +
...
template <typename T>
bool decode_32bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf,
                        const std::vector<int>& px_offset) {
...
bool decode_32bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf) {
    ...
    // 32bit channel data decoding to LidarFrame for key channel_index
    for (size_t row = 0; row < height; row++) {
        for (size_t col = 0; col < width; col++) {
            img(row, col) = static_cast<T>(row_pointers[row][(col * 4) + 0]) +
...
template <typename T>
bool decode_64bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf,
                        const std::vector<int>& px_offset) {
...
bool decode_64bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf) {
    ...
    // 64bit channel data decoding to LidarFrame for key channel_index
    for (size_t row = 0; row < height; row++) {
        for (size_t col = 0; col < width; col++) {
            uint64_t val = static_cast<uint64_t>(row_pointers[row][(col * 8) + 0]) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 1]) << 8u) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 2]) << 16u) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 3]) << 24u) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 4]) << 32u) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 5]) << 40u) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 6]) << 48u) +
                           (static_cast<uint64_t>(row_pointers[row][(col * 8) + 7]) << 56u);
...
template <typename T>
bool decode_16bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf,
                        const std::vector<int>& px_offset) {
...
bool decode_16bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf) {
    ...
    // 16bit channel data decoding to LidarFrame for key channel_index
    for (size_t row = 0; row < height; row++) {
        for (size_t col = 0; col < width; col++) {
            img(row, col) = static_cast<T>(row_pointers[row][(col * 2) + 0]) +
...
template <typename T>
bool decode_8bit_image(Eigen::Ref<img_t<T>> img, const EncodedFrameChannelData& channel_buf,
                       const std::vector<int>& px_offset) {
...
template bool decode_8bit_image<uint8_t>(Eigen::Ref<img_t<uint8_t>>, const EncodedFrameChannelData&,
