...
ReaderBase::ReaderBase(std::unique_ptr<OsfFile> osf_file, error_handler_t error_handler)
    : file_(std::move(osf_file)), error_handler_{std::move(error_handler)} {
    version_ = file_->version();

    if (version_.major > OsfFile::CURRENT_VERSION.major) {
        std::stringstream stream;
        stream << "The OSF file was created with schema version " << version_.to_string()
               << " but this reader supports up to major version " << OsfFile::CURRENT_VERSION.major
               << ". "
        ...
        std::stringstream stream;
        stream << "The OSF file was created with schema version " << version_.to_string()
               << ", but this reader only supports up to " << OsfFile::CURRENT_VERSION.major << "."
        ...
        error_handler_(ouster::sdk::core::Severity::OUSTER_WARNING, stream.str());
    }
...
bool ReaderBase::has_message_idx() const {
    return chunks_.has_message_idx();
};

bool ReaderBase::has_timestamp_idx() const {
    const auto streaming_infos = meta_store().find<StreamingInfo>();
    if (streaming_infos.empty()) {
        return false;
    }
    return std::all_of(streaming_infos.begin(), streaming_infos.end(), [](const auto& item) {
        return std::all_of(item.second->stream_stats().begin(), item.second->stream_stats().end(),
                           [](const auto& stats) {
                               return stats.second.message_count == 0 ||
...
std::string ReaderBase::metadata_id() const {
    ...
    if (auto metadata = impl::get_osf_metadata_from_buf(file_->get_metadata_chunk().data())) {
...
ts_t ReaderBase::start_ts() const {
    return start_ts_;
}

ts_t ReaderBase::end_ts() const {
    return end_ts_;
}

uint64_t ReaderBase::size() const {
    return file_->size();
}

const MetadataStore& ReaderBase::meta_store() const {
    return meta_store_;
}

bool ReaderBase::has_stream_info() const {
    return has_streaming_info_;
}

ouster::sdk::core::Version ReaderBase::version() const {
    return version_;
}

OsfFile& ReaderBase::file() const {
    return *file_;
}
...
void ReaderBase::read_metadata() {
    ...
    if (!file_->get_metadata_chunk().has_value()) {
        ...
        while (current_offset + sizeof(uint32_t) < file_->size()) {
            auto size_buf = file_->read(OsfOffset(current_offset, sizeof(uint32_t)));

            uint32_t size = *reinterpret_cast<const uint32_t*>(size_buf.data());

            // make sure we dont run past the end of the file
            if (current_offset + size + chunk_header_footer_size > file_->size()) {
                error_handler_(Severity::OUSTER_WARNING,
            ...
            // first read the size
            auto buf = file_->read(OsfOffset(current_offset, size + chunk_header_footer_size));

            auto chunk = ouster::sdk::osf::impl::gen::GetSizePrefixedChunk(buf.data());

            if (chunk->type() == nullptr) {
                throw std::runtime_error("Osf: File format too old to recover without metadata.");
            }

            // now recover metadata for each
            bool is_sensor_info =
                strcmp("ouster/v1/os_sensor/SensorInfoStreamMeta", chunk->type()->c_str()) == 0;

            bool is_collation =
                strcmp("ouster/v1/os_sensor/CollationStreamMeta", chunk->type()->c_str()) == 0;

            bool is_frame_set_source_metadata =
                strcmp("ouster/v1/os_sensor/ScanSourceMetadataStreamMeta",
                       chunk->type()->c_str()) == 0;
            ...
            for (size_t i = 0; i < messages->size(); i++) {
                ...
                if (is_sensor_info) {
                    ...
                    // add the lidar frame stream meta
                    LidarFrameStreamMeta meta(lidar_sensor_id);
                    meta.set_id(msg->frame_stream_id);
                ...
                } else if (is_frame_set_source_metadata && !meta_store_.get(stream_id)) {
                    FrameSetSourceMetadataStreamMeta meta;
            ...
            chunk_states.emplace_back(current_offset - chunk_start, ts_t{chunk_start_ts},
                                      ts_t{chunk_end_ts});
        ...
        // update chunk end to be the end of last chunk
        chunk_end = current_offset - chunk_start;
        ...
        for (auto& stat : info.stream_stats()) {
            if (!stat.second.receive_timestamps.empty()) {
                ...
                stat.second.end_ts = ts_t{stat.second.receive_timestamps.back()};
            }
        }
        ...
        for (uint32_t i = 0; i < entries->size(); ++i) {
            ...
            OsfBuffer entry_buf;
            uint64_t offset =
                reinterpret_cast<const uint8_t*>(entry) - file_->get_metadata_chunk().data();
        ...
        if ((metadata->chunks() != nullptr) && metadata->chunks()->size() > 0) {
            for (uint32_t i = 0; i < metadata->chunks()->size(); ++i) {
                ...
                chunk_states.emplace_back(chunk_offset->offset(), ts_t{chunk_offset->start_ts()},
    ...
    for (const auto& sci : streaming_info->chunks_info()) {
        chunks_.add_info(sci.first, sci.second.stream_id, sci.second.message_count);
    }
...
bool ReaderBase::verify_chunk(OsfOffset offset, OsfBuffer& buf) {
    ...
    if (chunk_state->status == ChunkValidity::UNKNOWN) {
        try {
            chunk_state->status =
                osf::impl::check_osf_chunk_buf(buf) ? ChunkValidity::VALID : ChunkValidity::INVALID;
            if (chunk_state->status != ChunkValidity::VALID) {
                error_handler_(Severity::OUSTER_WARNING,
                               "Invalid chunk at file offset " + std::to_string(offset.offset()));
            }
