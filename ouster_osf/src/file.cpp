...
#include "ouster/core/impl/logging.h"  // NOLINT(misc-include-cleaner)
...
Version deserialized_version(uint64_t integer_version) {
    ...
    switch (static_cast<OsfVersion>(integer_version)) {
...
OsfFile::OsfFile(std::string path) : state_(FileState::BAD), path_(std::move(path)) {}

std::string OsfFile::path() const {
    return path_;
}

OsfOffset OsfFile::metadata_offset() {
...
const ouster::sdk::core::Version OsfFile::CURRENT_VERSION{2, 2, 0};

ouster::sdk::core::Version OsfFile::version() {
    return deserialized_version(static_cast<uint64_t>(version_));
}

uint64_t OsfFile::serialized_version(const Version& parsed_version) {
...
bool OsfFile::operator!() const {
    return !good();
};

OsfFile::operator bool() const {
    return good();
};
...
void OsfFile::initialize_header_and_metadata() {
    ...
    OsfOffset header_offset{prefix_sized_offset.offset(),
    ...
    if (!impl::verify_osf_header_buf(header_chunk_.data(),
                                     header_chunk_.size() - osf::CRC_BYTES_SIZE)) {
    ...
    if (osf_header->status() != v2::HEADER_STATUS::VALID) {  // NOLINT(misc-include-cleaner)
        ouster::sdk::core::logger().warn("Osf: File metadata not marked as valid in header.");
        return;
    }

    // check that we have enough space for the metadata
    auto metadata_size = osf_header->file_length() - osf_header->metadata_offset();
    if (osf_header->metadata_offset() + metadata_size > size()) {
        ouster::sdk::core::logger().warn("Osf: Not enough space in file for metadata.");
        return;
    }
    ...
    if (!ouster::sdk::osf::impl::check_osf_metadata_buf(metadata_chunk_)) {
        metadata_chunk_ = {};
        ouster::sdk::core::logger().warn("Osf: Metadata verification has failed.");
        return;
    }
}

OsfOffset OsfFile::get_header_chunk_offset() {
    return header_offset_;
}

OsfOffset OsfFile::get_metadata_chunk_offset() {
    return metadata_offset_;
}

const OsfBuffer& OsfFile::get_header_chunk() {
    return header_chunk_;
}

const OsfBuffer& OsfFile::get_metadata_chunk() {
    return metadata_chunk_;
}
...
std::size_t std::hash<ouster::sdk::osf::OsfOffset>::operator()(
    ...
    return std::hash<uint64_t>()(key.offset()) ^ std::hash<uint64_t>()(key.size());
}
