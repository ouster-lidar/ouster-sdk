...
FrameChannelData PngLidarFrameEncoder::encode_field(const ouster::sdk::core::Field& field,
                                                    const std::vector<int>& px_offset) const {
    FrameChannelData buffer;
...
template <typename T>
bool PngLidarFrameEncoder::encode_8bit_image(FrameChannelData& res_buf,
                                             const Eigen::Ref<const img_t<T>>& img,
                                             const std::vector<int>& px_offset) const {
    return PngLidarFrameEncoder::encode_8bit_image<T>(res_buf, destagger(img, px_offset));
}
...
bool PngLidarFrameEncoder::encode_8bit_image(FrameChannelData& res_buf,
    ...
    impl::png_osf_write_start(png_ptr, png_info_ptr, res_buf, width, height, sample_depth,
                              color_type, compression_amount_);
    ...
    for (size_t row = 0; row < height; ++row) {
        ...
        png_write_row(png_ptr, reinterpret_cast<png_const_bytep>(row_data.data()));
    }
...
template <typename T>
bool PngLidarFrameEncoder::encode_16bit_image(FrameChannelData& res_buf,
                                              const Eigen::Ref<const img_t<T>>& img) const {
    const uint32_t width = static_cast<uint32_t>(img.cols());
    const uint32_t height = static_cast<uint32_t>(img.rows());
    ...
    std::vector<uint8_t> row_data(static_cast<size_t>(width) * 2);  // Gray, 16bit
    ...
    for (size_t row = 0; row < height; ++row) {
        for (size_t col = 0; col < width; ++col) {
            ...
            row_data[(col * 2) + 1] = static_cast<uint8_t>((key_val >> 8u) & 0xff);
        }
...
template <typename T>
bool PngLidarFrameEncoder::encode_24bit_image(FrameChannelData& res_buf,
                                              const Eigen::Ref<const img_t<T>>& img,
                                              const std::vector<int>& px_offset) const {
...
bool PngLidarFrameEncoder::encode_24bit_image(FrameChannelData& res_buf,
    ...
    std::vector<uint8_t> row_data(static_cast<size_t>(width) * 3);  // RGB, 8bit
    ...
    for (size_t row = 0; row < height; ++row) {
        for (size_t col = 0; col < width; ++col) {
            ...
            row_data[(col * 3) + 1] = static_cast<uint8_t>((key_val >> 8u) & 0xff);
            row_data[(col * 3) + 2] = static_cast<uint8_t>((key_val >> 16u) & 0xff);
        }
...
template <typename T>
bool PngLidarFrameEncoder::encode_32bit_image(FrameChannelData& res_buf,
                                              const Eigen::Ref<const img_t<T>>& img,
                                              const std::vector<int>& px_offset) const {
...
bool PngLidarFrameEncoder::encode_32bit_image(FrameChannelData& res_buf,
    ...
    std::vector<uint8_t> row_data(static_cast<size_t>(width) * 4);  // RGBA, 8bit
...
template <typename T>
bool PngLidarFrameEncoder::encode_64bit_image(FrameChannelData& res_buf,
                                              const Eigen::Ref<const img_t<T>>& img,
                                              const std::vector<int>& px_offset) const {
...
bool PngLidarFrameEncoder::encode_64bit_image(FrameChannelData& res_buf,
    ...
    std::vector<uint8_t> row_data(static_cast<size_t>(width) * 8);  // RGBA, 16bit
