...
flatbuffers::Offset<impl::gen::StreamingInfo> create_streaming_info(
    flatbuffers::FlatBufferBuilder& fbb, const std::map<uint64_t, ChunkInfo>& chunks_info,
    ...
    for (const auto& chunk_info_pair : chunks_info) {
        ...
        auto ci_offset =
            impl::gen::CreateChunkInfo(fbb, chunk.offset, chunk.stream_id, chunk.message_count);
    ...
    for (const auto& stream_stat_pair : stream_stats) {
        ...
        auto ss_offset = impl::gen::CreateStreamStats(
            fbb, stat.stream_id, stat.start_ts.count(), stat.end_ts.count(), stat.message_count,
            stat.message_avg_size, fbb.CreateVector<uint64_t>(stat.receive_timestamps),
            fbb.CreateVector<uint64_t>(stat.sensor_timestamps));
        stream_stats_vec.push_back(ss_offset);
    }

    auto si_offset = impl::gen::CreateStreamingInfoDirect(fbb, &chunks_info_vec, &stream_stats_vec);
    return si_offset;
}
...
std::string to_string(const ChunkInfo& chunk_info) {
    ...
    string_stream << "{offset = " << chunk_info.offset << ", stream_id = " << chunk_info.stream_id
                  << ", message_count = " << chunk_info.message_count << "}";
    return string_stream.str();
}

StreamStats::StreamStats(uint32_t s_id, ts_t receive_ts, ts_t sensor_ts, uint32_t msg_size)
    : stream_id{s_id},
      start_ts{receive_ts},
      end_ts{receive_ts},
      message_count{1},
...
void StreamStats::update(ts_t receive_ts, ts_t sensor_ts, uint32_t msg_size) {
    ...
    avg_size = avg_size + (static_cast<int>(msg_size) - avg_size) / static_cast<int>(message_count);
...
StreamingInfo::StreamingInfo(const std::vector<std::pair<uint64_t, ChunkInfo>>& chunks_info,
...
StreamingInfo::StreamingInfo(const std::map<uint64_t, ChunkInfo>& chunks_info,
                             const std::map<uint32_t, StreamStats>& stream_stats)
...
std::unique_ptr<MetadataEntry> StreamingInfo::from_buffer(const OsfBuffer& buf) {
    ...
    if ((streaming_info->chunks() != nullptr) && (streaming_info->chunks()->size() != 0u)) {
        std::transform(streaming_info->chunks()->begin(), streaming_info->chunks()->end(),
                       std::inserter(chunks_info, chunks_info.end()),
                       [](const impl::gen::ChunkInfo* chunk_info_ptr) {
                           return std::make_pair(
                               chunk_info_ptr->offset(),
                               ChunkInfo{chunk_info_ptr->offset(), chunk_info_ptr->stream_id(),
...
std::string StreamingInfo::repr() const {
    ...
    for (const auto& chunk_info_pair : chunks_info_) {
        ...
        chunk_info["offset"] = static_cast<uint64_t>(chunk_info_pair.second.offset);
    ...
    for (const auto& stat : stream_stats_) {
        ...
        stream_stat_json["start_ts"] = static_cast<uint64_t>(stat.second.start_ts.count());
        stream_stat_json["end_ts"] = static_cast<uint64_t>(stat.second.end_ts.count());
        stream_stat_json["message_count"] = static_cast<uint64_t>(stat.second.message_count);
        ...
        for (const auto& timestamp : stat.second.sensor_timestamps) {
            sensor_timestamps_json.emplace_back(static_cast<uint64_t>(timestamp));
        }
        for (const auto& timestamp : stat.second.receive_timestamps) {
            receive_timestamps_json.emplace_back(static_cast<uint64_t>(timestamp));
        }
