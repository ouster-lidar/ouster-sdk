...
bool check_osf_metadata_buf(const ouster::sdk::osf::OsfBuffer& buf) {
    ...
    auto verifier =
        flatbuffers::Verifier(buf.data(), buf.size() - ouster::sdk::osf::CRC_BYTES_SIZE);
    return check_prefixed_size_block_crc(buf, buf.size()) &&
           gen::VerifySizePrefixedMetadataBuffer(verifier);  // NOLINT(misc-include-cleaner)
}
...
bool check_osf_chunk_buf(const ouster::sdk::osf::OsfBuffer& buf) {
    ...
    auto verifier =
        flatbuffers::Verifier(buf.data(), buf.size() - ouster::sdk::osf::CRC_BYTES_SIZE);
    return check_prefixed_size_block_crc(buf, buf.size()) &&
           gen::VerifySizePrefixedChunkBuffer(verifier);  // NOLINT(misc-include-cleaner)
}
...
template std::vector<uint8_t> vector_from_fb_vector(const flatbuffers::Vector<uint8_t>* fb_vec);
template std::vector<double> vector_from_fb_vector(const flatbuffers::Vector<double>* fb_vec);
template std::vector<int> vector_from_fb_vector(const flatbuffers::Vector<int>* fb_vec);

// ============ File operations ==========================

uint64_t buffer_to_file(const uint8_t* buf, const uint64_t size, const std::string& filename,
                        bool append) {
    ...
    if (append) {
        file_stream.open(filename, std::fstream::out | std::fstream::app | std::fstream::binary);
    } else {
        file_stream.open(filename, std::fstream::out | std::fstream::trunc | std::fstream::binary);
    }

    if (file_stream.is_open()) {
        file_stream.write(reinterpret_cast<const char*>(buf), static_cast<int64_t>(size));
        if (!file_stream.good()) {
            return 0;
        }
        ...
        ouster::sdk::core::logger().error("ERROR: Failed to open {} for writing", filename);
    }
    return saved_size;
}
...
uint64_t start_osf_file(const std::string& filename) {
    ...
    auto header = ouster::sdk::osf::impl::gen::CreateHeader(
        ...
        static_cast<uint64_t>(
            OsfFile::serialized_version(ouster::sdk::osf::OsfFile::CURRENT_VERSION)),
        ouster::sdk::osf::impl::HEADER_STATUS::INVALID, 0, 0);
    header_fbb.FinishSizePrefixed(header, ouster::sdk::osf::impl::gen::HeaderIdentifier());
    return builder_to_file(header_fbb, filename, false);
    // NOLINTEND(misc-include-cleaner)
}

uint64_t finish_osf_file(const std::string& filename, const uint64_t metadata_offset,
                         const uint32_t metadata_size, const ouster::sdk::core::Version& version) {
    auto header_fbb = flatbuffers::FlatBufferBuilder(1024);
    auto header = ouster::sdk::osf::impl::gen::CreateHeader(
    ...
    file_stream.open(
        filename, std::fstream::out | std::fstream::in | std::fstream::ate | std::fstream::binary);
    if (file_stream.is_open()) {
        file_stream.seekp(0);
