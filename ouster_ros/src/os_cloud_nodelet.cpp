...
class OusterCloud : public nodelet::Nodelet {
   private:
    virtual void onInit() override {
        ...
        fixed_frame_ = pnh.param("fixed_frame_id", std::string{});
        if (!fixed_frame_.empty()) fixed_frame_ = tf_prefix + fixed_frame_;
        wait_for_transform_ = pnh.param("wait_for_transform", 0.01);
    ...
    void convert_scan_to_pointcloud_publish(std::chrono::nanoseconds scan_ts,
        ...
        for (int i = 0; i < n_returns; ++i) {
            if(fixed_frame_.empty()) {
                ...
                scan_to_cloud(xyz_lut, scan_ts, ls, cloud, i, listener_, fixed_frame_, sensor_frame, wait_for_transform_);
            }
