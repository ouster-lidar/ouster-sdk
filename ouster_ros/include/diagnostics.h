...
class Diagnostics {
    ...
    diagnostic_updater::TimeStampStatus timestamp_status;
...
public:
	...
	timestamp_status(diagnostic_updater::TimeStampStatusParam(-1, 5)),
        ...
        diagnostic_updater.add(timestamp_status);
    }
    ...
    void tick(const ros::Time& msg_time) {
        frequency_status.tick();
        timestamp_status.tick(msg_time);
    }
