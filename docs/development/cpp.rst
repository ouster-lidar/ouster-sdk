...
Building the example code requires a compiler supporting ``C++14`` and ``CMake 3.10`` or newer and the
``Eigen3`` and ``tins`` libraries with headers installed on the system. The sample visualizer also
requires the ``GLFW3`` library.
...
.. code:: console
   ...
   $ brew install cmake pkg-config eigen curl libtins glfw libpng flatbuffers libomp ceres-solver openssl@3 tbb zstd libzip zlib
