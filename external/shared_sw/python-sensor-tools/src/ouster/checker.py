...
class LidarState:
    ...
    def __init__(self, pkt_buf, ppm_tracker, lidar_mode, pf):
        ...
        self.pkt_data       = osl.OsLidarData(pkt_buf, pf)
        self.valid_view     = self.pkt_data.make_col_valid_view()
        self.col_id_view    = self.pkt_data.make_col_measurement_id_view()
        self.frame_id_view  = self.pkt_data.make_col_frame_id_view()
        self.ticks_view     = self.pkt_data.make_col_encoder_count_view()
        self.ts_view        = self.pkt_data.make_col_timestamp_view()

        self.range_view     = self.pkt_data.make_pixel_range_view()
        self.reflc_view     = self.pkt_data.make_pixel_reflectivity_view()
        self.signl_view     = self.pkt_data.make_pixel_signal_view()
        self.noise_view     = self.pkt_data.make_pixel_noise_view()
