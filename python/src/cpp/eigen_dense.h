...
*/
// Changed line 162 to remove unnecessary const breaking moves.
// clang-format off
...
struct type_caster<T, enable_if_t<is_eigen_plain_v<T> &&
    ...
    static handle from_cpp(T2 &&v, rv_policy policy, cleanup_list *cleanup) noexcept {
        ...
        if constexpr (std::is_pointer_v<T2>)
            return from_cpp_internal((T &) *v, policy, cleanup);
        else
            return from_cpp_internal((T &) v, policy, cleanup);
    }

    static handle from_cpp_internal(T &v, rv_policy policy, cleanup_list *cleanup) noexcept {
