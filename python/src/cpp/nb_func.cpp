...
// NOLINTBEGIN
// BEGINNOLINT
// OUSTER: Modified to include additional informations about ndarray types on
//         failure to match a function overload
...
namespace {
std::map<nanobind::dlpack::dtype_code, const char*> dtype_to_string = {
 { nanobind::dlpack::dtype_code::Int, "int"},
 { nanobind::dlpack::dtype_code::UInt, "uint"},
 { nanobind::dlpack::dtype_code::Float, "float"},
 { nanobind::dlpack::dtype_code::Bfloat, "bfloat"},
 { nanobind::dlpack::dtype_code::Complex, "complex"},
 { nanobind::dlpack::dtype_code::Bool, "bool"}
};

std::string to_string(const nanobind::dlpack::dtype& dtype) {
    auto str = dtype_to_string.find(static_cast<nanobind::dlpack::dtype_code>(dtype.code));
    if (str != dtype_to_string.end()) {
        return std::string(str->second) + std::to_string(dtype.bits);
    }
    return "unknown";
}
}
...
nb_func_error_overload(PyObject *self, PyObject *const *args_in,
    ...
    for (size_t i = 0; i < nargs_in; ++i) {
        ...
        // Give additional information about ndarray arguments to help the user
        if (nanobind::ndarray_check(args_in[i])) {
            auto obj = nanobind::borrow(args_in[i]);
            nanobind::ndarray<nanobind::ro> ndarray;
            bool success = nanobind::try_cast(obj, ndarray, false);
            if (!success) {
                // handle some strange ndarrays not well supported by nanobind
                buf.put_dstr(name.c_str());
            } else {
                // use nanobind's built in checking to find some properties
                nanobind::ndarray<> arr;
                bool writable = nanobind::try_cast(obj, arr, false);
                nanobind::ndarray<nanobind::c_contig, nanobind::ro> arr2;
                bool c_contig = nanobind::try_cast(obj, arr2, false);
                nanobind::ndarray<nanobind::f_contig, nanobind::ro> arr3;
                bool f_contig = nanobind::try_cast(obj, arr3, false);

                std::string realname = "ndarray[";
                realname += "dtype=" + to_string(ndarray.dtype());
                realname += ", order=";
                realname += c_contig ? "'C'" : (f_contig ? "'F'" : "'A'");
                realname += ", writable=";
                realname += writable ? "True" : "False";
                realname +=", shape=(";
                for (size_t i = 0; i < ndarray.ndim(); i++) {
                    realname += std::to_string(ndarray.shape(i));
                    if (ndarray.ndim() == 1 ||  i != ndarray.ndim() - 1) {
                        realname += ", ";
                    }
                }
                realname += ")]";
                buf.put_dstr(realname.c_str());
            }
        } else {
          buf.put_dstr(name.c_str());
        }
