...
using ouster::viz::MouseButton;
using ouster::viz::MouseButtonEvent;
...
void init_viz(py::module& m, py::module&) {
    m.doc() = R"(
    ...
    py::enum_<viz::MouseButton>(m, "MouseButton", py::arithmetic())
    ...
    py::enum_<viz::MouseButtonEvent>(m, "MouseButtonEvent", py::arithmetic())
    ...
    py::enum_<viz::EventModifierKeys>(m, "EventModifierKeys", py::arithmetic())
    ...
    py::class_<viz::PointViz>(m, "PointViz")
        ...
        .def(
            ...
            [](viz::PointViz& self,
               std::function<bool(const viz::WindowCtx& context,
                ...
                // pybind11 doesn't seem to deal with the rvalue ref arg
                // pybind11 already handles acquiring the GIL in the callback
                self.push_mouse_button_handler(std::move(f));
            },
        ...
        .def(
            ...
            [](viz::PointViz& self,
               std::function<bool(const viz::WindowCtx& context, double x,
                ...
                // pybind11 doesn't seem to deal with the rvalue ref arg
                // pybind11 already handles acquiring the GIL in the callback
                self.push_scroll_handler(std::move(f));
            },
        ...
        .def(
            ...
            [](viz::PointViz& self,
               std::function<bool(const viz::WindowCtx& context, double x,
                ...
                // pybind11 doesn't seem to deal with the rvalue ref arg
                // pybind11 already handles acquiring the GIL in the callback
                self.push_mouse_pos_handler(std::move(f));
            },
        ...
        .def(
            ...
            [](viz::PointViz& self,
               std::function<bool(const viz::WindowCtx& context)> f) {
                // pybind11 doesn't seem to deal with the rvalue ref arg
                // pybind11 already handles acquiring the GIL in the callback
                self.push_frame_buffer_resize_handler(std::move(f));
            },
