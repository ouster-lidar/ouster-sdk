...
def make_checker_board(square_size, reps):
    ...
    img_data = np.full((square_size, square_size), 0, dtype=np.float32)
...
def prepare_signal(frame):
    signal = frame.field(core.ChanField.REFLECTIVITY)
    signal = core.destagger(frame.sensor_info, signal)
    signal = np.divide(signal, np.amax(signal), dtype=np.float32)
    return signal


def example_1_2(point_viz, meta, frames):
    # [doc-stag-frame-fields-images]
    frame = next(frames)[0]
    ...
    # [doc-stag-frame-fields-images-labels]
    ...
    # [doc-etag-frame-fields-images-labels]

    # visualize
    point_viz.update()
    point_viz.run()

    remove_objs(point_viz, [range_img, signal_img, range_label, signal_label])


def example_2_0(point_viz, meta, frame):
    signal = prepare_signal(frame)

    # [doc-stag-frame-structured]
    cloud_frame = viz.Cloud(meta)
    cloud_frame.set_range(frame.field(core.ChanField.RANGE))
    cloud_frame.set_key(signal)
    point_viz.add(cloud_frame)
    # [doc-etag-frame-structured]

    # visualize
    point_viz.update()
    point_viz.run()

    remove_objs(point_viz, [cloud_frame])


def example_2_1(point_viz, meta, frame):
    signal = prepare_signal(frame)

    # [doc-stag-frame-unstructured]
    # transform frame data to 3d points
...
def example_2_2(point_viz, meta, frame):
...
def example_2_3(point_viz, meta, frame):
    # [doc-stag-lidar-frame-viz]
    # Creating LidarFrame visualizer (3D point cloud + field images on top)
    ls_viz = viz.LidarFrameViz([meta], point_viz)

    # adding frame to the lidar frame viz
    ls_viz.update([frame])
    ...
    # visualize
    # update() is not needed for LidarFrameViz because it's doing it internally
    point_viz.run()
    # [doc-etag-lidar-frame-viz]

    point_viz.pop_mouse_pos_handler()
    point_viz.pop_mouse_button_handler()
    point_viz.pop_frame_buffer_resize_handler()
    point_viz.pop_key_handler()


def example_3(point_viz):
    # [doc-stag-lidar-frame-viz-labels]
    # Adding 3D Labels
    ...
    # [doc-etag-lidar-frame-viz-labels]
...
def main():
    ...
    # =======================================
    print("Ex 1.2:\tImages and Labels: Lidar Frame Fields as Images")
    example_1_2(point_viz, meta, frames)
    ...
    example_2_0(point_viz, meta, frame)

    # ===============================================================
    print("Ex 2.1:\tPoint Clouds: As Unstructured Points")
    example_2_1(point_viz, meta, frame)

    # =======================================================
    print("Ex 2.2:\tPoint Clouds: Custom Axes Helper as Unstructured Points")
    example_2_2(point_viz, meta, frame)

    # ===============================================================
    print("Ex 2.3:\tPoint Clouds: the LidarFrameViz class")
    example_2_3(point_viz, meta, frame)
