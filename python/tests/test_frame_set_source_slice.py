...
@pytest.fixture
def sliceable_fixture(frame_set_source_path):
    frame_set_source = open_source(frame_set_source_path, index=True)
    ref_ids = [s[0].frame_id for s in frame_set_source]  # register ref ids
    return frame_set_source, ref_ids
...
def test_slicing_level_1(sliceable_fixture, start, stop, step) -> None:
    frame_set_source, ref_ids = sliceable_fixture
    s = slice(start, stop, step)
    print(s)
    ss_slice = frame_set_source[s]
    print(ss_slice)
...
def test_slicing_negative_step_throws(sliceable_fixture, start, stop, step) -> None:
    frame_set_source, _ = sliceable_fixture
    s = slice(start, stop, step)
    with pytest.raises(ValueError) as ex:
        _ = frame_set_source.slice(s)
...
def test_index_from_level_1_slice(sliceable_fixture, start, stop, step, idx) -> None:
    frame_set_source, ref_ids = sliceable_fixture
    s = slice(start, stop, step)
    print(frame_set_source[s][idx])
    assert frame_set_source[s][idx][0].frame_id == ref_ids[s][idx]
...
def test_index_slicing_out_of_range_throws(sliceable_fixture, start, stop, step, idx):
    ...
    with pytest.raises(IndexError) as ex:
        _ = frame_set_source[s][idx]
    assert str(ex.value) == "Indexed past the end of the frame set source."
