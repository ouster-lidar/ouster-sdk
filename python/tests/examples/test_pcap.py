...
def test_pcap_query_frame_lists_fields(capsys, test_data_dir: Path):
    pcap_file = _pcap_path(test_data_dir)
    if not pcap_file.exists():
        pytest.skip("PCAP fixture missing")

    pcap_examples.pcap_query_frame(str(pcap_file))
    out = capsys.readouterr().out
...
def test_pcap_to_ply_writes_expected_points(tmp_path, test_data_dir: Path):
    ...
    # limit to first frame from pcap
    frame_set_source = pcap.PcapFrameSetSource(str(pcap_file), sensor_info=[metadata])
    frame, = next(iter(frame_set_source))
    assert frame is not None
    expected_xyz = core.XYZLut(metadata)(frame.field(core.ChanField.RANGE))
...
def test_pcap_3d_one_frame_returns_xyz_without_visualizing(test_data_dir: Path):
    try:
    ...
    xyz = pcap_examples.pcap_3d_one_frame(str(pcap_file), visualize=False)
